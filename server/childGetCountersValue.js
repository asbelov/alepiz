...
var debugCounters = require('../serverDebug/debugCounters');
...
if(messageListenerAlreadyStarted) return log.error('Trying to initialize this child process more than one time. Exiting');
...
history.connect(childID, function () {
    debugCounters.connect(startMessageListener);
});
...
function startMessageListener() {
    ...
    var childProc = new thread.child({
                ...
                async.each(data.expressions, function (variable, callback) {
                    ...
                    calc(variable.expression, variables, counterID,
                        ...
                        if (updateEventState === undefined || !updateEventState !== !result) {
                            ...
                            // result can be an undefined when counter has not updated event expression
