 ...
 *      updateEventMode: ({number}),
 *      parentVariables: (*|{"<var1>": "<value1>"}),
 *      updateEventExpression: ({string}),
 *      counterID: ({number}),
 *      taskCondition: (number|{number}|*),
 *      collector: ({string}|*),
 *      counterName: ({string}|*),
 ...
 *      objectName ({string}),
 *      objectCounterID: ({number}),
 *      objectID: ({number}),
 *      parentOCID: ({number}|*),
 *      countersObjects: {},
 *      variablesDebugInfo: {},
 *      cache: {
 *          variable: Map<any, any>,
...
function getVars(param, callback) {
    ...
    variables.PARENT_COUNTER_NAME = param.parentCounter === undefined ? '' : param.parentCounter;
    ...
    getUpdateEventState(variables, param, function (err, whyNotNeedToCalculateCounter) {
        ...
        getCounterParameters(param, variables,function (err, counterParameters) {
            ...
            var parentCounter = param.countersObjects.counters ? param.countersObjects.counters.get(param.counterID) : null;
            if(param.counterID === 4) console.log('!!getVars find dependent: counter', parentCounter, '; dependedUpdateEvents', parentCounter.dependedUpdateEvents.size, '; vars: ', variables);
            if (!parentCounter || !parentCounter.dependedUpdateEvents.size) {
...
function getUpdateEventState(variables, param, callback) {
    ...
    getVar(null, variables, param, function(err, result) {
        ...
        var whyNotNeedToCalculateCounter =
            processUpdateEventExpressionResult(result, param.updateEventMode, param.prevUpdateEventState);

        if (param.prevUpdateEventState === undefined || Boolean(param.prevUpdateEventState) !== Boolean(result)) {
            variables.UPDATE_EVENT_TIMESTAMP = Date.now();
        ...
        if (param.variablesDebugInfo.UPDATE_EVENT_STATE) {
            if (whyNotNeedToCalculateCounter) {
                param.variablesDebugInfo.UPDATE_EVENT_STATE.result += ' (No calculation required: ' + whyNotNeedToCalculateCounter + ')';
            } else {
                param.variablesDebugInfo.UPDATE_EVENT_STATE.result += ' (Calculation required)';
                param.variablesDebugInfo.UPDATE_EVENT_STATE.important = true;
            }
        ...
        //if(param.counterID === 172) console.log('!!getUpdateEventState end: ', param.updateEventExpression, '=', result, '; whyNotNeedToCalculateCounter: ', whyNotNeedToCalculateCounter, '; vars: ', variables);
...
function getVar(initVariableName, variables, param, callback) {
    ...
    function getVariableValue(variableName, callback) {
        ...
        // set variable value to null to know that the variable has already been tried to be calculates
        ...
        if(variableName) {
            ...
            if(!variable) {
                return callback(new Error(param.objectName + ' (' + param.counterName + ' #' + param.counterID +
                    '): variable ' + variableName + ' is not defined. Cache props: ' +
