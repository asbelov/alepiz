...
router.post('/mainMenu', function(req, res/*, next*/) {
    ...
    else if(functionName === 'getObjectsByID') getObjectsByIDs(req.body.IDs, req.session.username, req.body.filterNames, req.body.filterExpression, sendBackResult);
 ...
 */
function filterObjectsByInteractions(objectsNamesStr, user, filterNames, filterExpression, callback) {
 ...
 * objects like [{name: ..., id: ..., description: ..., sortPosition:...}, {...}, ...]
 */
function searchObjects(initSearchStr, user, filterNames, filterExpression, callback) {
    ...
    var searchStr = '%'+initSearchStr.
            ...
            replace(/[&|].{0,5}([&|])/g, '$1'). // remove patterns smaller than 6 characters between logical operators
            replace(/^.{0,4}[&|]/, ''). // remove search patterns smaller than 5 characters at the forward
            replace(/[&|].{0,4}$/, '')+'%'; // remove patterns smaller than 5 characters at the backward
