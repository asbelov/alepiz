...
try {
    var exportedModule = require(requiredFile);
} catch (err) {
    parentPort.postMessage({
...
if(typeof exportedModule === 'function') var init = 'function';
else {
    init = {};
    for (var name in exportedModule) {
        init[name] = typeof exportedModule[name];
    }
}
...
parentPort.on('message', msg => {
    ...
    var data = typeof exportedModule === 'function' ? exportedModule : exportedModule[msg.name];
    if (!data) return;

    if (msg.sessionID) module.sessionID = msg.sessionID;
