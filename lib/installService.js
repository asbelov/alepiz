...
var logDir = confLog.get('dir') || 'logs';
    ...
    if (process.argv[2] === "--install" || process.argv[2] === "-i") {
        var options = {
            ...
            nodeArgs: [
                ...
                // change also in public/javaScript/init.js: var maxUrlLength=32767
                '--max-http-header-size=' +  + String(conf.get('httpHeaderSize') || 32767),
                '--report-on-fatalerror',
                '--report-uncaught-exception',
                '--redirect-warnings=NodejsWarning.log',
                '--diagnostic-dir=' + path.join(logDir, confLog.get('reportDir') || 'nodejs'),
                '--report-dir=' + path.join(logDir, confLog.get('reportDir') || 'nodejs'),
            ],
...
function installLog(message) {
    var exitLogFileName = confLog.get('exitLogFileName') || 'exit.log';
