 ...
 * @param {Boolean} [dontThrowOnError] dont throw on error
 *
 * @example
 * const Conf = require('../lib/conf);
 * const conf = new Conf(<fileName>, <initReloadTimeInterval>)
 */
function Conf(initFileName, initReloadTimeInterval, dontThrowOnError) {
    ...
    function loadFile(initFileName, initReloadTimeInterval) {
        ...
        try {
            ...
            lastReloadTime = 0;
            simpleLog('[get configuration] Can\'t read configuration file ', initFileName, ': ', err.message);
            return false;
        }
        ...
        try {
            ...
            lastReloadTime = 0;
            simpleLog('[get configuration] Can\'t parse configuration file ', initFileName, ': ', err.message);
            return false;
        }
     ...
     * @returns {*|Object} - required parameter or undefined if not found
     * @memberOf Conf
     */
    ...
    function simpleLog() {
        var logFileWithoutSuffix = path.join(__dirname, '..', 'logs', 'errorConf.log');
        var label = 'lib:conf';
        var level = 'E';
        ...
        streamLog.on('finish', function() {
            if (!dontThrowOnError && !configuration.size) throw new Error(message);
        });
    }
}
