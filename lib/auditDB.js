...
var auditDBFile = confLog.get('auditDBold') || 'db/audit.tsv';
var lastRecordID = 0;
...
auditDB.insertRecords = function(messageObj) {
    if(!messageObj || !messageObj.sessionID || !auditDBFile) return;

    writeStream.write(Buffer.from(createRawRecord(messageObj)));
    lastRecordID += messageObj.length;
};
...
auditDB.getRecords = function(lastRecordID, sessionsIDs, callback) {
    ...
    if(maxRecordsCnt !== parseInt(String(maxRecordsCnt), 10) || maxRecordsCnt <= 0 ) maxRecordsCnt = 1000;
...
function createRawRecord(messageObj) {
    ...
    var rawRecord = Buffer.concat([Buffer.from([messageObj.sessionID, messageObj.timestamp, messageObj.level,
        messageObj.label].join('\u0009')), Buffer.from([0x09]), message]);
