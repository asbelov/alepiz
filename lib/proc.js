...
if(require.main === module) calcMaxMessageSize();

/*
options = {
    childrenNumber: <int> children number. if 0 then number will be equal to CPUs number.
        If childrenNumber is an array, then number will be equal to array length and %:childID:% will be set to array item
    ...
    simpleLog: <boolean> if true, use serverLog\simpleLog instead standard log function
...
proc.parent = function (options, callback) {
    ...
    function runChild(callback) {
        ...
        child.safeSend = new ProcMessage(child, log).safeSend;
        child.id = childID;
        ...
        child.on('exit', function(exitCode, signal) {
            ...
            if(stopInProgress || killInProgress) checkForChildrenCnt(errExitCode);
                ...
                // exitCode 10 will reserve for prevent to restart child process
        ...
        child.on('message', function(data) {
            if(data.message === 'initComplete') {
                if(data.err) {
                    ...
                    if(options.restartAfterErrorTimeout) {
                        var restartTimer = setTimeout(runChild, options.restartAfterErrorTimeout);
                        restartTimer.unref();
                    }
                }
...
options.simpleLog: <boolean> if true, use serverLog/simpleLog instead standard log function
...
proc.child = function(options) {
    ...
    if(options.affinity) {
        ...
        if(res === -1) {
            log.error('Error while set affinity to ', (options.affinity >>> 0).toString(2),
                ' (0x', Number(options.affinity).toString(16), '). CPUS: ', os.cpus().length);
        }
        else {
    ...
    var messageID,
        pr = new ProcMessage(process, log),
