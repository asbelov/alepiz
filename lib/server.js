...
/**
 * Created by asbel on 11.10.2015.
 */
...
function initServerCommunication() {
    ...
    function startServer(message, callback) {
        ...
        if(stopServerInProgress) {
            log.exit('A request was received to start the server for operation "', message,
                '", but a server stop operation is in progress. Cancel server start');
            log.warn('A request was received to start the server for "', message,
                '", but a server stop operation is in progress. Cancel server start');
    ...
    function stopServer(message, callback, isQuiet) {
        ...
        if(startServerInProgress) {
            ...
            stopServerDelayTimer = setTimeout(stopServer, 1000, message, callback);
            return;
        }
