...
var serverType = conf.get('server:type');
if (serverType === 'newServer') return module.exports = require('../server/serversManager');
...
function runServer(serverID, serversNumber) {
    ...
    var processedObjects = new Map(),
        ...
        updateCacheTimeout = ( cfg.updateCacheTimeout || updateCacheInterval/1000 ) * 1000;
    ...
    var fullUpdateCacheInterval = cfg.fullUpdateCacheInterval * 1000;
    ...
    function updateCache() {
        if((!needToUpdateCache.size &&
            ...
            (updateCacheInProgress && Date.now() - updateCacheInProgress < updateCacheTimeout)) return;

        if (updateCacheInProgress) {
            log.warn('The previous cache update operation was not completed in ', Math.round((Date.now() - updateCacheInProgress)/60000) , 'min');
        }
        ...
        createCache(updateMode, function(err, cache) {
            ...
            removeCounters(function () {
                var sendingComplete = false, gettingTopObjectsComplete = false;
                if(cache) {
                    ...
                    sendMessageToChildren.sendToAll(cache, function (err) {
                        ...
                        if (gettingTopObjectsComplete) updateCacheInProgress = 0;
                ...
                async.eachLimit(objectsAndCountersForUpdate, 10000, function (message, callback) {
                    ...
                    if (!properties.length) {
                        if(sendingComplete) updateCacheInProgress = 0;
                        gettingTopObjectsComplete = true;
                        return;
                    }
    ...
    function sendRestartToParent() {
        ...
        if(serverProcess) serverProcess.send({ restart: 1 });
    }
    ...
    function getCountersValues(properties, parentVariables) {
        ...
        removeCounters(function () {
            ...
            // for getting each value every 2ms.
            // Doesn't work correctly and adds many data gaps in history
            //var runCounter = 0, minProcessingTime = 2; // ms
            filteredProperties.forEach(function (property) {
                getCounterValue(property, parentVariables);
                //setTimeout(getCounterValue,runCounter++ * minProcessingTime, property, parentVariables);
            });
        });
