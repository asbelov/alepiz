...
function calcExpression(initExprString, initVariables, callback){
    ...
    if(typeof initVariables === 'function') {
        callback = initVariables;
        initVariables = null;
    } else if(!initVariables || typeof initVariables !== 'object' || !Object.keys(initVariables).length) initVariables = null;
    ...
    log.debug('Starting calculate expression: ', initExprString, '; variables: ', initVariables);
    ...
    replaceQuotedStrings(initExprString, function(err, quotedStrings, exprString) {
        ...
        replaceVariables(exprString, initVariables, quotedStrings, function(err, quotedStrings, exprString, unresolvedVariables) {
            if (err) return callback(err);
...
function calc(initExprString, expr, quotedStrings, functions, callback) {
    //log.debug('initExprString: ', initExprString, '; expr: ', expr, '; quotedStrings: ', quotedStrings, '; functions: ', functions);
    var functionDebug = [];
...
function replaceFunctions(str, callback) {
    ...
    funcPositions.forEach(function(func, idx) {
    ...
    //log.debug('Functions: ', myFunctions, ', str: ', str, ', initStr: ', initStr);
