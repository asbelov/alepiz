...
function calcExpression(initExprString, variables, getVariableValue,  callback){
    ...
    replaceQuotedStrings(exprString,function(err, quotedStrings, exprString) {
        ...
        // !!! replaceVariables() must locate after replaceQuotedStrings() because in another case we can't process
        // variable in quotes, f.e. like this: getValueFromJSONStr('%:PARENT_VALUE:%', "hostname" )
...
function convertResult(result) {
    if(typeof result === 'boolean' || isNumeric(result)) result = Number(result);
...
function resolveVarAndFunc(operation, quotedStrings, functions, getVariableValue, variables, unresolvedVariables, callback) {
    ...
    if (operation.type === 'func') {
        executeFunction(operation, quotedStrings, functions, getVariableValue, variables, unresolvedVariables, function (err, result, debug) {
            if (err) return callback(err);
        ...
        if(typeof getVariableValue === 'function') {
            ...
            getVariableValue(operation.data, function (err, result) {
                ...
                if ((err || result === undefined) && operation.canBeUnresolved) {
    ...
    while(true) {
        ...
        if((!operator || !operators[operator].unary) && prevOperator !== ')') {
            if (isNumeric(operation)) {
                ...
                if(quotedStrings[idx] && /^"%:.+:%"$/.test(quotedStrings[idx])) {
                    var varName = quotedStrings[idx].replace(/^"%:(.+):%"$/, '$1');
    ...
    for(var i=0, quotedStringsIdx = 0; i < str.length; i++) {
        ...
        if (str[i] === '"' || str[i] === "'" || str[i] === '`') {
            ...
            if (endOfQuotedString === -1)
                return callback(new Error('Error in expression: [' + initStr + ']: unfinished quoted string from ' + str.substring(i)));
...
function replaceVariables(str, variables, quotedStrings, callback) {
    ...
    for(var i = 1, newStr = strParts[0], unresolvedVariables = []; i < strParts.length; i++) {
        ...
        if(!name) newStr += '%:' + strParts[i];
            ...
            quotedStrings.push('"%:' + addUnresolved + name + ':%"');
    ...
    //log.info('!!! vars Unresolved: ', unresolvedVariables, '; str: ', str, ' => ', newStr, '; quotedStrings: ', quotedStrings, '; variables: ', variables);
...
function executeFunction(funcObj, quotedStrings, functions, getVariableValue, variables, unresolvedVariables, callback){
    ...
    async.eachOf(funcObj.parameters, function(parameter, index, callback){
        splitExpression(parameter, functions, quotedStrings, function(err, expr) {
            ...
            if(expr.length > 1) {
                return calc(parameter, expr, quotedStrings, functions, getVariableValue, variables, unresolvedVariables, function(err, result) {
                    if(err) return callback(err);
            ...
            if(type === 'var') {
                if(typeof getVariableValue === 'function') {
                    ...
                    return getVariableValue(expr[0].data, function (err, result) {
                        ...
                        // if the variable is unresolved, we pass to the function not 0, but null, so that the function can
                        // distinguish an unresolved variable
                        if((result === undefined || result === null) && !expr[0].canBeUnresolved) {
                            return callback(new Error('Received undefined result for ' + expr[0].data));
                        }
                    ...
                    else return callback(new Error('Unresolved variable ' + expr[0].data));
                }
            }
            ...
            if(type === 'func') {
                ...
                return executeFunction(newFuncObj, quotedStrings, functions, getVariableValue, variables, unresolvedVariables,
                    function (err, debug) {
                    if (err) return callback(err);
...
function calcFunction(functionName, parameters, callback){
    if(functionName in calcFunctions) {
        ...
        return callback(new Error('Can\'t executing unknown function '+ functionName+ '('+ (parameters !== undefined && parameters.length ? parameters.join(', ') : parameters)+ ')'));
    }
}