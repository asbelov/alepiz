    ...
    var clientIPC = new IPC.client(cfg, function(err, message, isConnected) {
        ...
        function STMT(messageID) {
            this.run = function() {
                ...
                if(callback) stmts[messageID].run(param, callback);
                else stmts[messageID].run(param);
            };
            ...
            this.finalize = function(callback) {
                if(stmts[messageID]) {
                    if(typeof callback === 'function') stmts[messageID].finalize(callback);
                    else stmts[messageID].finalize();

                    delete stmts[messageID];
            ...
            function runStmt(func, args) {
                ...
                if(param && callback) stmts[messageID][func](param, callback);
                else if(param && !callback) stmts[messageID][func](param);
                else if(!param && callback) stmts[messageID][func](callback);
                else stmts[messageID][func]();
            }
        }
        ...
        function DB() {
            ...
            this.prepare = function() {
                ...
                if(param && callback) stmts[messageID] = initDB.prepare(sql, param, callback);
                else if(!param && callback) stmts[messageID] = initDB.prepare(sql, callback);
                else if(param && !callback) stmts[messageID] = initDB.prepare(sql, param);
                else stmts[messageID] = initDB.prepare(sql);
