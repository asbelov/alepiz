...
var cleanUpCallbackStackTimer = setTimeout(cleanUpCallbackStack, 3600000);
cleanUpCallbackStackTimer.unref();
 ...
 * @param {Object} options - options {moduleName: <moduleName>, {<function>: permanentCallback: <true|false>}}
 * @param {function(Error)|function(null, object)} callback - callback(err, obj) - return error or obj like
 * @example
 * options:
 * {
 *     moduleName: the label used in the log file
 *     module: node.js module object for send module.sessionID to the thread
 *     <function>: <function> — the name of the exported function to apply the
...
module.exports = function (requiredFile, options, callback) {
    ...
    var parentModule = options && typeof options.module === 'object' ? options.module : {};
    const threadLabel = '[' + path.basename(path.dirname(requiredFile)) + ':' +
        path.basename(requiredFile) + moduleName + '] ';
        ...
        if(exportedObj === 'function') {
            returnedObj.func = function () {
                ...
                for (var mod = parentModule; mod; mod = mod.parent) {
                    if (mod.sessionID) {
                        postData.sessionID = Number(mod.sessionID);
                        break;
                    }
                }
                ...
                if (typeof args[args.length - 1] === 'function') {
                    ...
                    try {
                        callbackStack.set(messageID, {
                            ...
                            timestamp: Date.now(),
                            permanent:
                                options && options.function && typeof options.function === 'object' ?
                                    options.function.permanentCallback : false,
                        });
                    }  catch (e) {
                        log.throw('Can\'t add callback: ', e.message,
                            '; options: ', options,
                            '; Number of the Map elements: ', callbackStack.size);
                    }
                }
                postData.args = args;
                        ...
                        if (typeof args[args.length - 1] === 'function') {
                            ...
                            try {
                                callbackStack.set(messageID, {
                                    ...
                                    timestamp: Date.now(),
                                    permanent:
                                        options && options[_name] && typeof options[_name] === 'object' ?
                                            options[_name].permanentCallback : false,
                                });
                            } catch (e) {
                                log.throw('Can\'t add callback for ', _name, ': ', e.message,
                                    '; options: ', options,
                                    '; Number of the Map elements: ', callbackStack.size);
                            }
                        }
