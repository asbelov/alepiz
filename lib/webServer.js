...
var webSecrets = require('../lib/webSecrets');
...
// uncomment after placing your favicon in /public
    ...
    webSecrets.get(function (err, web) {
        if(err) {
            ...
            setTimeout(process.exit, 10000, 12);
            return;
        }
...
// no stack traces leaked to user
        ...
        var port = Number(process.env["app_port"] || process.env.PORT || conf.get('httpsPort'));
        try {
            var sslOptions = {
                key:  fs.readFileSync(path.join(__dirname, '..', conf.get('privatePath') ||
                    'config', conf.get('httpsKeyFile') || 'key.pem')),
                cert: fs.readFileSync(path.join(__dirname, '..', conf.get('privatePath') ||
                    'config', conf.get('httpsCertFile') || 'cert.pem')),
            }

            log.info('Starting HTTPS ', process.pid, ' on port ', port || 443);
            https.createServer(sslOptions, app).listen(port || 443);
        } catch (e) {
            port = Number(conf.get('httpPort') || conf.get('port'));
