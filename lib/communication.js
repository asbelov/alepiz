...
var communicationDir = conf.get('communicationMedia:dir') || 'communication';
var configFileName = conf.get('communicationMedia:configuration') || 'config.json';
var serverFileName = conf.get('communicationMedia:server') || 'server.js';
var defaultConfigID = conf.get('communicationMedia:defaultConfigID') || 'default';
var reloadSourceEveryTime = conf.get('communicationMedia:reloadSourceEveryTime') || false;
var communicationMedia = {};

/*
param: {
    message: { feel the field required for communication media. f.e. for email it can be:
        from:
        to:
        cc:
        bcc:
    ...
    priorities: [<num1>, <num2>,..] array of priorities ID from userCommunicationPriorities.priority.
    mediaID: <mediaID> - string. if set mediaID then priorities will be skipped
    configID: configID or don't set for default
    sender: userName from users.name table
    rcpt: array of userNames from users.name table,
    text: message text
    variables: variables {<name>: <value>, ....}, will be replaced in param.text and in all message parameters
}
 */
communication.send = function(param, callback) {
    ...
    getMedias(param.sender, null, function (err, sender) {
        ...
        if(param.media && Array.isArray(param.priorities) && param.priorities.length) {
            ...
            async.each(medias, function (mediaID, callback) {
                getConfig(mediaID, param.configID, param.message, param.variables, function(err, transport, message, configID) {
                    ...
                    if(reloadSourceEveryTime && require.resolve(mediaSource) && require.cache[require.resolve(mediaSource)]) {
                        ...
                        delete communicationMedia[mediaSource];
                    }

                    if(!communicationMedia[mediaSource]) {
                        try {
                            log.warn('Attaching communication media source file ', mediaSource);
                            communicationMedia[mediaSource] = require(mediaSource);
                        } catch (err) {
                    ...
                    try {
                        communicationMedia[mediaSource].send({
                            configID: configID,
                            transport: transport,
                            message: message,
...
function replaceVariables(str, variables) {
    ...
    var res = calc.variablesReplace(str, variables);
