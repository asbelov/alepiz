...
const logRunner = require('../serverLog/logServerRunner');
const dbServer = require('../serverDB/dbServerRunner');
const initDB = require('../models_db/createDB/createAllDB');
const modifyDB = require('../models_db/createDB/modifyDB');
...
function onDestroy(callback) {
    ...
    history.dump(function() {
        ...
        var t = setTimeout(process.exit, 15000, 6);
        t.unref(); // process.exit(6) for search
    });
...
function scheduleRestart() {
    ...
    var t = setTimeout(restart, d.getTime() - Date.now());
    t.unref();
}
        ...
        dbServer.start(function (err) {
            ...
            initDB(function (err) {
                ...
                backupServer.start(function (err) {
                    ...
                    modifyDB(function (err) {
                        if (err) {
                            log.exit('Error while making changes in the DataBase stricture: ', err.stack);
                            log.disconnect(function () { process.exit(11) });
                            return;
                        }

                        log.info('Starting history storage');
                        history.start(function (err) {
                            if (err) {
                            ...
                            taskServer.stop(function () {
                                ...
                                actionServer.stop(function () {
                                    ...
                                    dbServer.stop(function () {
                                        ...
                                        logRunner.stop(function () {
                                            log.exit('All services are stopped');

                                            dontRunDestroy = true;
                                            // process.exit(6) for search
                                            var t1 = setTimeout(process.exit, 15000, 6);
                                            t1.unref();

                                            var t2 = setTimeout(callback, 500);
                                            t2.unref();
                                        });
                                    })
                                });
