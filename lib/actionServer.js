        ...
        async.parallel([
            ...
            if(executionMode === 'makeTask'){
                ...
                if(!ajax[ajaxSource] || prms.updateAction) {
                    try {
                        log.info('Attaching file for run ajax ', ajaxSource, (prms.updateAction ? '. Required action update. Cached data was deleted.' : ' at a first time.'));
                ...
                if(!launchers[launcherSource] || prms.updateAction) {
                    try {
                        log.info('Attaching launcher file ', launcherSource, (prms.updateAction ? '. Required action update. Cached data was deleted.' : ' at a first time.'));
