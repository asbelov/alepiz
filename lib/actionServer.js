...
function runServerProcess() {
    ...
    new IPC.server(cfg, function(err, msg, socket, callback) {
        ...
        if(msg && msg.msg === 'runAction') return runActionsQueue(msg.param, callback);
        if(msg && msg.msg === 'createSession') return createSession(msg.param, callback);
    ...
    */
    ...
    function runActionsQueue(param, callback) {
        actionsQueue.push({
            prms: param,
            callback: callback
        });
        ...
        if (memUsage * 1.5 > maxMemSize && processedActions && param.user === systemUser) {
            try {
                global.gc();
    ...
    actions.getConfiguration(actionID, function(err, actionCfg){
        ...
        async.parallel([
            ...
            if(executionMode === 'makeTask'){
                ...
                if(param.updateAction && require.resolve(ajaxSource) && require.cache[require.resolve(ajaxSource)]) delete require.cache[require.resolve(ajaxSource)];
                if(!ajax[ajaxSource] || param.updateAction) {
                    try {
                        log.info('Attaching file for run ajax ', ajaxSource, (param.updateAction ? '. Required action update. Cached data was deleted.' : ' at a first time.'));
                ...
                try {
                    ajax[ajaxSource](param.args, callback);
                } catch (err) {
                ...
                if(param.updateAction && require.resolve(launcherSource) && require.cache[require.resolve(launcherSource)]) delete require.cache[require.resolve(launcherSource)];
                if(!launchers[launcherSource] || param.updateAction) {
                    try {
                        log.info('Attaching launcher file ', launcherSource, (param.updateAction ? '. Required action update. Cached data was deleted.' : ' at a first time.'));
