...
IPC.server = function(cfg, callback) {
    ...
    setTimeout(cleanUpCallbackStack, cleanUpCallbacksPeriod, callbackStack, keepCallbackInterval);
    ...
    function getNextSocketIndex() {
        if(currentSocketIndex >= connectedSockets.length) currentSocketIndex = 0;
        // connected indicates whether it is still possible to send and receive messages from a child process
        if(connectedSockets[currentSocketIndex]) {
            socketFindingIteration = 0;
            return currentSocketIndex++;
        }

        if(++socketFindingIteration > connectedSockets.length) {
            socketFindingIteration = 0;
            return;
        }
        return getNextSocketIndex();
    }

    function sendToAll (data, callback) {
        if(typeof callback !== 'function') callback = function(err) { if(err) log.warn(err.message); };

        async.eachOf(connectedSockets, function (connectedSocket, socketIndex, callback) {
            if(connectedSocket) send(data, socketIndex, 0, false, callback);
            else callback();
        }, callback);
    }
    ...
    function send (data, socketIndex, sentMessageID, permanent, callback) {
        ...
        if(socketIndex === undefined || socketIndex === null) socketIndex = getNextSocketIndex();
        ...
        if(!connectedSockets[socketIndex]) {
            log.warn('Socket with index ' + socketIndex + ' not exist');
            return callback(new Error('Socket with index ' + socketIndex + ' not exist'));
        }

        if(sentMessageID) {
            callbackStack[connectedSockets[socketIndex].remotePort + '_' + sentMessageID] = {
                timestamp: Date.now(),
                permanent: permanent,
                func: callback
            };
        ...
        dataSender(null, connectedSockets[socketIndex], data, sentMessageID, callback);
    }
    ...
    function stop(callback) {
        ...
        server.close(function(err) {
            ...
            connectedSockets = [];
            isServerRunning = 0;
            if(err) callback(new Error('IPC: can\'t close server socket: ' + err.message));
            else callback();
        });
    }
    ...
    function start() {
        ...
        server = net.createServer(function(socket) {
            ...
            var socketIndex = connectedSockets.length;
            ...
            socket.on('timeout', function() {
                connectedSockets[socketIndex] = null;
                socket.end();
                if(stopInProgress) return;
            ...
            dataReceiver(socket, function(err, receivedMessageID, result) {
                ...
                if(result === stopMessage) {
                    ...
                    setTimeout(process.exit, 2000, 10); // for simple searching exitCode 10
                    return;
                }

                var callbackID = socket.remotePort + '_' + receivedMessageID;
                if (callbackStack[callbackID] && typeof callbackStack[callbackID].func === 'function') {

                    if(callbackStack[callbackID].removed)
                        //log.warn('Callback for messageID ', receivedMessageID, ', message: ', resultStr , ' was removed at ', new Date(callbackStack[callbackID].removed).toLocaleString());

                    callbackStack[callbackID].func(err, result, socketIndex);
                    if (!callbackStack[callbackID].permanent) {
                        //callbackStack[callbackID].removed = Date.now();
                        delete callbackStack[callbackID];
                    }
                    return;
                ...
                callback(err, result, socketIndex, function (err1, message) {
                    ...
                    if(!receivedMessageID) log.warn('Sending back message with ID "', receivedMessageID, '": ', message);
                    var returnedError = err ? (err1 ? new Error(err.stack + ' AND ' + err1.stack) : err) : (err1 ? err1 : null);
                    dataSender(returnedError, socket, message, receivedMessageID);
                });
            });
        });
        ...
        server.on('error', function(err) {
            ...
            if(!attemptsCnt--) {
                log.error(message);
                log.exit(message);
                return setTimeout(process.exit, 10000, 4);
...
connectAndSendMessage = function(serverAddress, serverPort, localAddress, message, callback) {
    ...
    socket.on('connect', function() {
        dataSender(null, socket, message, null, function(err) {
            socket.end();
            callback(err);
        });
    });
...
IPC.client = function(cfg, callback) {
    ...
    if(id) id = 'client:' + id;
        ...
        if(module.parent.parent) {
            id = 'client:' + path.basename(module.parent.parent.filename, '.js') + '-' + path.basename(module.parent.filename, '.js');
        } else id = 'client:' + path.basename(module.parent.filename, '.js');
    } else id = 'client';

    var log = new IPC.log(id || 'Client');
    var storageFileName = path.join(__dirname, '..', conf.get('IPCStoragePath') || 'db', id + storageFileSuffix + '.ipc');
    ...
    function send (data, sentMessageID, permanent, callback) {
        ...
        dataSender(null, socket, data, sentMessageID, callback);
    }
    ...
    function connect() {
        ...
        socket.on('connect', function() {
            ...
            if(unsentData.length === unsentMessagesCnt) {
            ...
            dataReceiver(socket, function(err, receivedMessageID, result) {
                if (callbackStack[receivedMessageID] && typeof callbackStack[receivedMessageID].func === 'function') {

                    //if(callbackStack[receivedMessageID].removed) log.warn('Callback for messageID ', messageID, ', message: ', result, ' was removed at ', new Date(callbackStack[receivedMessageID].removed).toLocaleString());

                    callbackStack[receivedMessageID].func(err, result);
                    if (!callbackStack[receivedMessageID].permanent) {
                        //callbackStack[receivedMessageID].removed = Date.now();
                        delete callbackStack[receivedMessageID];
                    }
                    return;
            ...
            if(storageFD) {
                ...
                fs.close(storageFD, function (err) {
                    ...
                    fs.unlink(storageFileName, function (err) {
                        if (err) log.warn('Can\'t delete file ' + storageFileName + ' with unsent data: ' + err.message);
                        else log.info('Delete file with unsent data ' + storageFileName);
                    });
                });
            }
...
function dataSender(err, socket, data, sentMessageID, callback) {
...
function dataReceiver(socket, callback) {
...
// cleanup unused callbacks
function cleanUpCallbackStack(callbackStack, keepCallbackInterval) {
    var now = Date.now();
...
function socketErrorsHandler(err, socketErrorCounter, log) {
    ...
    if(socketErrorCounter.num > socketErrorCounter.maxNum &&
        ...
        log.error(message);
        log.exit(message);
        setTimeout(process.exit, 10000, 4);
    }
}