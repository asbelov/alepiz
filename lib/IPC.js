...
var logFileWithoutSuffix = path.join(String(confLog.get('dir') || 'logs'),
    String((confLog.get('lib:IPC:file') ? confLog.get('lib:IPC:file') : 'IPC.log')));
...
var logLevel = levelOrder[confLog.get('logLevel') || 'I'];
...
function runClusterWorker() {
    ...
    function processMessageWorker(msg) {
        ...
        if(msg.f === 'sendBack') {
            var callbackObj = callbackStack.get(msg.id);
            if(callbackObj && typeof callbackObj.func === 'function') {
                // don't send empty data back to client
...
function writeLog(level, args, label) {
    ...
    var dateStr = String((timestamp.getMonth()+1) + '.0' +
        timestamp.getDate()).replace(/0(\d\d)/g, '$1') + ' ';
    var timeStamp = String('0'+ timestamp.getHours() + ':0'+ timestamp.getMinutes() + ':0' +
            timestamp.getSeconds()).replace(/0(\d\d)/g, '$1') +
    ...
    if (!streamLog) { // open log stream first time
        try {
            streamLog = fs.createWriteStream(newLogFile, {flags: 'a'});
            logFile = newLogFile;
        } catch (e) {
            console.log('Can\'t create new stream from file descriptor first time for ' + newLogFile + ': ' + e.message);
        }
    } else { // log rotation
        if (logFile !== newLogFile) {
            try {
                var newStreamLog = fs.createWriteStream(newLogFile, {flags: 'a'});
            } catch (e) {
                streamLog.write('Can\'t create new stream from file descriptor for ' + newLogFile + ': ' + e.message);
            }
            ...
            if (newStreamLog) {
                try {
                    ...
                    newStreamLog.write('Can\'t end streamLog or close file descriptor for ' + logFile + ': ' + e.message);
                }
                streamLog = newStreamLog;
                logFile = newLogFile;
            }
        }
    }

    try { streamLog.write(message); } catch(e) {}
...
function ProcMessage(proc, log) {
    ...
    function safeSend(message, messageData, callback) {
        ...
        proc.send(message, function (err) {
            if (err) {
                err = new Error('Error while sending data: ' + err.messages + ': ' + JSON.stringify(message).substring(0, 500) + '...');
