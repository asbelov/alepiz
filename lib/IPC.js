...
const sentMaxAttempts = 10; // use const!!!
var errNumSendingData = 0, unsentDataNum = 0;
var printingErrorsWasInitializing = false;
...
IPC.ProcMessage = ProcMessage; // export safeSend function for cluster IPC
IPC.sentMaxAttempts = sentMaxAttempts; // export for use in cluster IPC
...
IPC.server = function(cfg, callback) {
    var serverAddress = cfg.serverAddress,
        ...
        id = cfg.id,
        cleanUpCallbacksPeriod = cfg.cleanUpCallbacksPeriod || 3600000,
        keepCallbackInterval = cfg.keepCallbackInterval || 1209600000, // 14 days
        isServerRunning = 0,
    ...
    setInterval(cleanUpCallbackStack, cleanUpCallbacksPeriod, callbackStack, keepCallbackInterval, log);
    if(cfg.exitOnNoClients === parseInt(cfg.exitOnNoClients, 10) && cfg.exitOnNoClients > 1000) {
        setTimeout(function () {
            ...
            log.warn('No clients are connected to IPC worker. Exiting after 3 sec...');
            setTimeout(process.exit, 3000); // process.exit();
        }, cfg.exitOnNoClients);
    }
    ...
    function start() {
        ...
        server = net.createServer(function(socket) {
            ...
            ++clientsNum;
            log.info('Client connected: ', socket.remoteAddress, ':', socket.remotePort, '->', serverAddress, ':', serverPort);
            attemptsCnt = maxAttemptsCnt;
...
/*
    workersNum - number of workers
 */
IPC.cluster = function (cfg, callback) {
    var id = cfg.id;
    cfg.cluster = true;

    if(id) id = 'cluster:' + id;
    else if(module.parent) id = 'cluster:' + path.basename(module.parent.filename, '.js');
    else id = 'server';

    var log = new IPC.log(id || 'Cluster');

    var workersNum = cfg.workersNum !== parseInt(String(cfg.workersNum), 10) || cfg.workersNum < 1 ?
        os.cpus().length : Number(cfg.workersNum);

    var workers = [],
        procMessages = new Map(),
        initializedWorkersNum = 0,
        messageID,
        firstMessageID = 1,
        callbackStack = new Map(),
        cleanUpCallbacksPeriod = cfg.cleanUpCallbacksPeriod || 3600000,
    ...
    log.info('Starting IPC cluster workers...');
    for (var i = 0; i < workersNum; i++) {
        var worker = cluster.fork();
        workers.push(worker);
        procMessages.set(worker, new ProcMessage(worker, log));
    }

    setInterval(cleanUpCallbackStack, cleanUpCallbacksPeriod, callbackStack, keepCallbackInterval, log);

    this.sendToAll = function (data, callback) {
        sendToAllWorkers('sendToAll', data, callback);
    };

    this.sendAndReceive = function (data, socketIndex, callback) {
        sendToAllWorkers('sendAndReceive', data, socketIndex, callback);
    };

    this.sendAndPermanentReceive = function (data, socketIndex, callback) {
        sendToAllWorkers('sendAndPermanentReceive', data, socketIndex, callback);
    };

    this.stop = function(callback) {
        sendToAllWorkers('stop', callback);
    };

    this.destroy = function() {
        sendToAllWorkers('destroy');
    };

    this.send = function (data, socketIndex, callback) {
        sendToAllWorkers('send', data, socketIndex, callback);
    };

    this.send = function (data, callback) {
        sendToAllWorkers('send', data, null, callback);
    };
    ...
    this.send = function (data, callback) {
        messageID = getNewMessageID(messageID, firstMessageID);
        sendToWorker(getNextWorker(), messageID, 'send', [data], callback);
    };

    function getNextWorker() {
        if(currentWorkerID >= workers.length) currentWorkerID = 0;
        return workers[currentWorkerID++];
    }
...
*/
    ...
    cluster.on('fork', function (worker) {
        worker.on('message', function (msg) {
            processMessageMaster(msg, worker);
        });

        messageID = getNewMessageID(messageID, firstMessageID);
        sendToWorker(worker, messageID, 'init', cfg);
    });

    function sendToAllWorkers() {
        ...
        workers.forEach(function (worker) {
            sendToWorker(worker, myMessageID, func, args);
        });

        if(typeof callback === 'function') {
            callbackStack.set(messageID, {
                timestamp: Date.now(),
                permanent: func === 'sendAndPermanentReceive',
                func: callback,
            });
        }
    }

    function sendToWorker(worker, messageID, func, args) {
        var procMessage = procMessages.get(worker);
        procMessage.safeSend({
            id: messageID,
            f: func,
            args: args,
        });
    ...
    function processMessageMaster(msg, worker) {
        //log.info('MSG: ', msg)
        if(!msg) return;
        if(msg.init) { // init
            if(msg.err) log.error('Error initialize worker: ', msg.err);
            log.info('Init worker ', msg.init, ': ', ++initializedWorkersNum, '/', workersNum)
            if(initializedWorkersNum === workersNum) {
                log.info('All IPC workers are started and initialized');
                callback(msg.err, null, -1); // send -1 as socketIndex
            }

        } else if (msg.msg && msg.sIdx) { // rcv message
                callback(null, msg.msg, msg.sIdx, function (sendBackErr, sendBackMessage) {
                    sendToWorker(worker, msg.id, 'sendBack', [sendBackErr, sendBackMessage]);
                });
        } else {  // all other
            var callbackObj = callbackStack.get(msg.id);
            if (callbackObj && typeof callbackObj.func === 'function') {
                callbackObj.timestamp = Date.now();
                Array.isArray(msg.args) ? callbackObj.func.apply(this, msg.args) : callbackObj.func();
                if (!callbackObj.permanent) {
                    //callbackObj.removed = Date.now();
                    callbackStack.delete(msg.id);
                }
            }
        }
    }
...
/*
history(IPC.client) => IPC.cluster worker => IPC.cluster master (historyCache) =>
IPC.cluster worker => IPC.client(history)

history:
    history.getByIdx = function(id, offset, cnt, maxRecordsCnt, getByIdxCallback)
client:
clientIPC.sendAndReceive( {
        ...
        recordsType: 0,
    }, sendAndReceiveCallback = getByIdxCallback);
    sendAndReceiveCallback => callbackStack

worker:
dataReceiver(log, socket, callback)
    new IPC.server (msg.cfg, function processMessageWorker(err, message, socket, messageCallback)
    run processMessageWorker(err, result, socketIndex, function messageCallback(err1, message))
        process.send({
             id: msg.id,
             msg: message,
             sIdx: socket,
 ...
 master:
    master(cfg, callback)
    processMessageMaster() => callback => history.processMessage(message, socketIdx, messageCallback)

historyCache:
    processMessage(null, message, socketIdx, messageCallback)
    processMessage() -> getByIdx(id, last, cnt, maxRecordsCnt, recordsType, messageCallback)
    messageCallback()
...
worker:
    run messageCallback(err, result)
        dataReceiver(log, socket, callback) {
            callback(err, result, socketIndex, function messageCallback(err1, message) {
                ...
                dataSender(log, returnedError, socket, message, receivedMessageID);
            });
        }
client:
    run callbackStack => sendAndReceiveCallback(err, result)
history:
    run sendAndReceiveCallback(err, result) = getByIdxCallback(err, result)

-> w (gIdx) -> m (gIdx => callback(err, res))
 */
function runClusterWorker() {
    var exitHandler = require('../lib/exitHandler');
    exitHandler.init(null, process.argv[2]);
    ...
    var serverIPC,
        ...
        callbackStack = new Map(),
        messages = new Set(),
        processMessagesInProgress = 0;

    process.on('message', processMessageWorker);

    function processMessageWorker(msg) {
        //log.info('callbackStack size', callbackStack.size, '; msg: ', msg);
        var procMessage = new ProcMessage(process, log);

        if(msg.f === 'sendBack') {
            var callbackObj = callbackStack.get(msg.id);
            if(callbackObj && typeof callbackObj.func === 'function') {
                // don't sent empty data back to client
                // only cleanup callbacks stack
                if(Array.isArray(msg.args) && (msg.args[0] !== null || msg.args[1] !== null)) {
                    callbackObj.timestamp = Date.now();
            ...
            var cfg = msg.args;
            serverIPC = new IPC.server(cfg, function(err, returnedMsg, socketIndex, messageCallback) {
                if(err) log.error(err.message);
                ...
                if(socketIndex === -1) {
                    procMessage.safeSend({
                        ...
                        if(err) log.error(errModuleInfo + 'Can\'t send init message to parent in cluster: ', err.message);


                        var cleanUpCallbacksPeriod = cfg.cleanUpCallbacksPeriod || 3600000,
                            keepCallbackInterval = cfg.keepCallbackInterval || 1209600000; // 14 days
                        setInterval(cleanUpCallbackStack, cleanUpCallbacksPeriod, callbackStack, keepCallbackInterval, log);
                    });
                    ...
                    if (typeof messageCallback === 'function') {
                        try {
                            ...
                            /*
                            !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                            if you see this error add a callback() to all IPC.client functions
                            if you don't need to pass the data back to client, call the callback without parameters
                             */
                            log.error('Can\'t add callback for messageID: ', myMessageID, '; callbacks stack size', callbackStack.size, ': ', e.message);
                        }
                    }
            ...
            var args = msg.args || [];
            // add callback
            args.push(function (err, result, socketIndex) {
                procMessage.safeSend({
                    id: msg.id,
                    err: err,
                    msg: result,
            ...
            serverIPC[msg.f].apply(this, args);
        }
    }
}
...
function dataSender(log, err, socket, data, sentMessageID, callback) {
    ...
    socket.write(message, function(err) {
        if(typeof callback === 'function') setTimeout(callback, 0, err);
        else if(err) log.warn('Can\'t write to socket: ', err, ': id: ', sentMessageID, ': ', data);
    });
...
function dataReceiver(log, socket, callback) {
    ...
    function dataParser(data) {
        ...
        while (data.length >= headerLength) {
            ...
            setTimeout(callback, 0, result.err ? new Error(result.err) : null, receivedMessageID, result.data);
...
function cleanUpCallbackStack(callbackStack, keepCallbackInterval, log) {
    var now = Date.now();
    var err = new Error('Timeout ' + keepCallbackInterval / 60000 + 'min occurred while waiting for IPC callback calling');
...
// safe send data to destination process
function ProcMessage(proc, log) {
    //var messages = [];
    //var sendInProgress = 0;

    this.safeSend = safeSend;
    this.safeSend = function (data, callback) { safeSend(data, null, callback); }

    // messageData used for send any data to callback function
    function safeSend(message, messageData, callback) {
        if (!proc || typeof proc.send !== 'function') {
            var err = new Error('Possible destination process was destroyed for sending:' + JSON.stringify(message).substring(0, 500) + '...')
            if (typeof callback === 'function') callback();
            else log.error(err.message);
            return;
        }

        proc.send(message, function (err) {
            if (err) {
                var err = new Error('Error while sending data: ' + err.messages + ': ' + JSON.stringify(message).substring(0, 500) + '...');
                if (typeof callback === 'function') callback(err, messageData);
                else log.error(err.message);
            }
            if (typeof callback === 'function') callback(null, messageData);
        });
    }

    /*
    if (!printingErrorsWasInitializing) {
        printingErrorsWasInitializing = true;
        setInterval(function () {
            if (!errNumSendingData && !unsentDataNum) return;
            log.info('Number of erroneous attempts to send messages: ', errNumSendingData, '; unsent messages: ', unsentDataNum);
            errNumSendingData = unsentDataNum = 0;
        }, 30000);
    }

    function verySafeSend(message, messageID, callback) {
        if (!proc || typeof proc.send !== 'function') {
            var err = new Error('Possible destination process was destroyed for sending:' + JSON.stringify(message).substring(0, 500) + '...')
            if (typeof callback === 'function') callback();
            else log.error(err.message);
        ...
        if (message !== undefined || typeof callback === 'function') {
            messages.push({
                data: message,
                callback: callback,
            });
        } // else send data from messages queue

        if (sendInProgress) return;

        sendInProgress = Date.now();

        // clone message to new array and clear queue
        var myMessages = messages.slice();
        messages = [];

        async.eachSeries(myMessages, function (message, callback) {
            var isDataSent = false;

            async.eachSeries(new Array(sentMaxAttempts).fill(0), function (i, callback) {
                // isDataSent was set to true, but callback was not calling
                if (isDataSent) {
                ...
                if (!proc || typeof proc.send !== 'function') {
                    return callback(new Error('Possible process was destroyed for sending: ' + JSON.stringify(message.data).substring(0, 500) + '...'));
                }

                proc.send(message.data, function (err) {
                    if (err) {
                        log.warn('Error while sending data: ', err.messages, ', try again: ', JSON.stringify(message.data).substring(0, 500) + '...');
                        ++errNumSendingData;
                        return callback()
                    }
                    isDataSent = true;
                ...
                if (isDataSent) {
                    callback();
                    if (typeof message.callback === "function") message.callback(null, messageID);
                } else {
                    ++unsentDataNum;
                    if (!err) {
                        err = new Error('Max attempts (' + sentMaxAttempts + ') reached while sending data: ' +
                            JSON.stringify(message.data).substring(0, 500) + '...');
                    }
                    if (typeof message.callback === "function") message.callback(err, messageID);
                    callback();
                }
            });
        }, function () {
            sendInProgress = 0;
            if (messages.length) safeSend();
        });
    }
    */
}

if(cluster.isWorker) runClusterWorker();