...
IPC.server = function(cfg, callback) {
    ...
    setInterval(function () {
        ...
        if(prevCallbackStackSize && callbackStackSize && increase > 0) {
            log.info('Callback stack size: ', callbackStackSize, ' increase: ', increase);
        }
        ...
        server = net.createServer(function(socket) {
            ...
            socket.on('timeout', function() {
                ...
                log.debug('IPC server socket closed due timeout ' + (socketTimeout / 1000) + ' sec : ' +
                    socket.remoteAddress + ':' + socket.remotePort + '->' + localAddress + ':' + serverPort);
            });

            socket.on('error', function(err) {
                if(stopInProgress) return;
                socketErrorsHandler(new Error('IPC server socket error: ' + err.message + ' for: ' + socket.remoteAddress +
                    ':' + socket.remotePort + '->' + localAddress + ':' + serverPort), socketErrorCounter, log);
            });
            ...
            socket.on('close', function(hadError) {
                ...
                if(hadError) {
                    log.warn('IPC server: socket closed due to a transmission error to client for: ' + socket.remoteAddress +
                        ':' + socket.remotePort + '->' + localAddress + ':' + serverPort);
            ...
            dataReceiver(log, socket, function(err, receivedMessageID, result) {
                ...
                if (callbackObj && typeof callbackObj.func === 'function') {
                    ...
                    log.warn('IPCServer: can\'t find callback for messageID ', receivedMessageID, '/', messageID, ', message: "', result, '"');
...
IPC.client = function(cfg, callback) {
    var serverAddress = cfg.serverAddress,
        ...
        reconnectDelay = Number(cfg.reconnectDelay) === parseInt(cfg.reconnectDelay, 10) ?
    ...
    returnedObject.send = function(data, callback) { send(data, 0, false, callback) };
    ...
    returnedObject.sendAndPermanentReceive = function(data, callback) {
    ...
    returnedObject.isConnected = function () { return socket && !socket.pending}

    if(socket && !socket.pending) return callback();
    if(connectOnDemand) callback(null, null, returnedObject);
    ...
    function send (data, sentMessageID, permanent, callback) {
        ...
        connect(function() {
            ...
            if(!socket || socket.pending || socket.connecting) {
    ...
    function connect(callback) {
        if(socket && (!socket.pending || socket.connecting)) return callback();
        dontReconnectToServer = false;
        ...
        socket.on('timeout', function () {
            socket.end();
            if(stopInProgress || !reconnectDelay || dontReconnectToServer) return;
        ...
        if(connectionTimeout) {
            var connectTimeoutTimer = setTimeout(function () {
                socket.end();
                // return returnedObject for make possible to send data and store unsent data using
                // IPC saveUnsentData() function while connection will not established
                callback(new Error('IPC connection timeout ' + (connectionTimeout / 1000) +
                    'sec occurred. Reconnecting after ' + (reconnectDelay / 1000) + 'sec: ' +
                    socket.localAddress + ':' + socket.localPort + '->' + serverAddress + ':' + serverPort
                    ), null, returnedObject);
        ...
        socket.on('connect', function() {
            ...
            dataReceiver(log, socket, function(err, receivedMessageID, result) {
                ...
                if (callbackObj && typeof callbackObj.func === 'function') {
                    ...
                    log.warn('IPCClient: can\'t find callback for messageID ', receivedMessageID, '/', messageID, ', message: "', result, '"');
        ...
        socket.on('error', function (err) {
            if(connectTimeoutTimer !== undefined) clearTimeout(connectTimeoutTimer);
            if(stopInProgress) return; // don't print error when stop in progress
            socketErrorsHandler(new Error('IPC client socket error: ' + err.message + ' for: ' +
                socket.localAddress + ':' + socket.localPort + '->' + serverAddress + ':' + serverPort),
                socketErrorCounter, log);

            callback(new Error('IPC client socket error: ' + err.message + ' for: ' +
                socket.localAddress + ':' + socket.localPort + '->' + serverAddress + ':' + serverPort +
                '; reconnecting after ' + (reconnectDelay / 1000) + ' sec'));
        ...
        socket.on('close', function (hadError) {
            ...
            if (hadError && (socket && !socket.pending)) {
        ...
        socket.on('end', function () {
            ...
            log.info('IPC client socket closed: ' +
                socket.localAddress + ':' + socket.localPort + '->' + serverAddress + ':' + serverPort,
                '; reconnecting after ', (reconnectDelay / 1000), ' sec');

            callback(new Error('IPC client socket closed: ' +
                socket.localAddress + ':' + socket.localPort + '->' + serverAddress + ':' + serverPort +
                '; reconnecting after ' + (reconnectDelay / 1000) + ' sec'));
    ...
    function saveUnsentData(data, callback) {
        ...
        zlib.brotliCompress(stringifiedData, function(err, compressedData) {
            ...
            if((socket && !socket.pending) && savedMessagesCnt + unsentData.size === unsentMessagesCnt) {
                sendUnsentData(function(err) {
                    if(err) log.warn(err.message);
                });
            }
    ...
    function sendUnsentData(callback) {
        ...
        async.whilst(function() {
            ...
            fs.read(storageFD, sizeBuf, 0, 4, filePos, function (err, bytesRead, sizeBuf) {
                ...
                Stack: RangeError [ERR_OUT_OF_RANGE]: The value of "length" is out of range. It must be >= 0. Received -2103406993
...
function dataSender(log, err, socket, data, sentMessageID, callback) {
    ...
    try {
        ...
        if(!stringifiedData || !stringifiedData.length || stringifiedData.length > maxMessageLen) {
            err1 = new Error('Can\'t send data. Message too long: ' + stringifiedData.length + '/ ' + maxMessageLen+ ': ' + stringifiedData.substring(0, 512) + '...');
...
function dataReceiver(log, socket, callback) {
    ...
    /** Parse receiving data and return parsed result (callback(err, result)
     *
     * @param {Buffer} data
     * @returns {*}
     */
