...
IPC.server = function(cfg, callback) {
    var serverAddress = cfg.serverAddress,
        ...
        restartListenerOnErrorTime = cfg.restartListenerOnErrorTime || 5000, // delay between attempts to restart the listener on error
        isServerRunning = 0,
...
IPC.client = function(cfg, callback) {
    var serverAddress = cfg.serverAddress,
        ...
        reconnectDelay = Number(cfg.reconnectDelay) === parseInt(cfg.reconnectDelay, 10) ? cfg.reconnectDelay : 5000,
        disconnectOnIdleTime = Number(cfg.disconnectOnIdleTime) === parseInt(cfg.disconnectOnIdleTime, 10) ?
            cfg.disconnectOnIdleTime : 900000,
    ...
    function connect(callback) {
        ...
        socket.on('error', function (err) {
            if((!socket || socket.pending) || stopInProgress) return; // don't print error when server is not initialized
