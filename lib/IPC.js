 ...
 * @property {function(void)} isConnected return connection status to server (true|false)
 * @property {function(void)} getSocketStatus return object with socket status like
 *  {pending: socket.pending, destroyed: socket.destroyed, connecting: socket.connecting} or undefined if socket
 *  was not initialised.
 * @property {function(callback)} stop run onStop() function if exist, than run disconnect() method
 * @property {function()|function(callback)} disconnect disconnect from the IPC.server
 * @property {function()|function(callback)} destroy alias to the disconnect method
 * @property {function(data: *, [callback])} send send data to the IPC.server
 * @property {function(data: *, callback)} sendAndReceive send data to the IPC.server and receive answer using
 *  callback(err, receivedData) function
 * @property {function(data: *, sendExtOptions, [callback])} sendExt send or sendAndReceive data to
 ...
 * @param {boolean} [cfg.logToConsole=false] write a log messages to the console. not in IPC.log. Default false
 ...
 * @param {number} [cfg.disconnectOnIdleTime=0] if there is no sending or receiving data during the specified time,
 *  disconnection from the server will be performed. If a reconnection is required, the connectOnDemand = true
 *      parameter must be set. By default 0 - do not disconnect
 ...
 * @param {number} [cfg.connectionTimeout=0] Sets the socket connection timeout in ms. If a reconnection timeout
 *  is set, a reconnection attempt will be made after the connection timeout expires. Default 0 - disabled
...
IPC.client = function(cfg,
    ...
    var serverAddress = cfg.serverAddress,
        ...
        cleanUpCallbacksPeriod = Number(cfg.cleanUpCallbacksPeriod) ===
            parseInt(cfg.cleanUpCallbacksPeriod, 10) &&
            Number(cfg.cleanUpCallbacksPeriod) ? Number(cfg.cleanUpCallbacksPeriod) : 3600000,
        ...
        disconnectOnIdleTime = Number(cfg.disconnectOnIdleTime) ===
            parseInt(String(cfg.disconnectOnIdleTime), 10) ? Number(cfg.disconnectOnIdleTime) : 900000,
        ...
        maxCallbacksCnt = Number(cfg.maxCallbacksCnt) === parseInt(cfg.maxCallbacksCnt, 10) ?
            Number(cfg.maxCallbacksCnt) : 10000,
        keepAlive = Number(cfg.keepAlive) === parseInt(String(cfg.keepAlive), 10) && Number(cfg.keepAlive) > 1000 ?
            Number(cfg.keepAlive) : 5000,
    ...
    returnedObject.sendExt = function(data, sendExtOptions, callback) {
        var myMessageID = 0;
    ...
    returnedObject.getSocketStatus = function () {
        if(!socket) return;
        return {
...
function writeLog(level, args, label) {
    ...
    var timeStamp = timestamp.toLocaleTimeString() + '.' +
        String('00' + timestamp.getMilliseconds()).replace(/^0*?(\d\d\d)$/, '$1');

    var message = timeStamp + '[' + label + TID_PID + ']' + level + ': ' + logStr;
...
function closeOldLogFile() {
    ...
    d.setHours(0, 1, 0, 0);
    var t = setTimeout(closeOldLogFile, d - (new Date()));
    t.unref();
}
