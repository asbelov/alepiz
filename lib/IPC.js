...
IPC.server = function(cfg, callback) {
    ...
    function sendToAll (data, callback) {
        if(typeof callback !== 'function') callback = function(err) { if(err) log.info(err.message); };
    ...
    function send (data, socketIndex, sentMessageID, permanent, callback) {
        ...
        if(socketIndex === undefined) {
            log.info('Children sockets are not exist. Waiting...');
    ...
    function start() {
        ...
        server = net.createServer(function(socket) {
            ...
            socket.on('close', function(hadError) {
                ...
                if(hadError) {
                    log.info('IPC server: socket closed due to a transmission error to client' +
                        printConnectionInfo(socket, cfg, true));
                }
            });
            ...
            dataReceiver(log, socket, function(err, receivedMessageID, result) {
                ...
                if (callbackObj && typeof callbackObj.func === 'function') {
                    ...
                    //if(callbackObj.removed) log.info('Callback for messageID ', receivedMessageID, ', message: ', resultStr , ' was removed at ', new Date(callbackObj.removed).toLocaleString());
                ...
                callback(err, result, socketIndex, function (err1, message) {
                    //log.info('!!!', receivedMessageID, ' SendBack:', message)
                    if(err1) log.info(err1.message);
...
IPC.client = function(cfg, callback) {
    ...
    function send (data, sentMessageID, sendExtOptions, callback) {
        ...
        //log.info('!!!', sentMessageID, ' Send:', sendExtOptions, ';', data);

        // here try to connect. f.e. when connectOnDemand = true.
        connect(function() {

            var canRunCallback = typeof callback === 'function';
            if(sendExtOptions.sendAndReceive && canRunCallback) {
            ...
            if(socket && !socket.pending && !socket.connecting && !socket.destroyed) {
                dataSender(log,null, socket, data, sentMessageID, function(err) {
                    // if no error or got an error but dontSaveUnsentMessage === true
                    if(!err || sendExtOptions.dontSaveUnsentMessage) {
                        // run callback when no response is expected to be sent
                        // it is equal to if(!sendExtOptions.sendAndReceive || typeof savedCallback !== 'function') callback();
                        if(canRunCallback) callback(err);
                        else if(err) log.info(err.message); // err can be if dontSaveUnsentMessage === true
                        return;
                    }

                    // Got an error and now try to save unsent data for send letter
                    saveUnsentData({data: data, messageID: sentMessageID, options: sendExtOptions}, err,
                        function (err1) {
                            ...
                            // run callback when no response is expected to be sent
                            // it is equal to if(!sendExtOptions.sendAndReceive || typeof savedCallback !== 'function') callback();
                            if(canRunCallback) callback(commonErr);
                        });
                });
                return;
            }
            ...
            if(!sendExtOptions.dontSaveUnsentMessage || !socket) {
                ...
                saveUnsentData({data: data, messageID: sentMessageID, options: sendExtOptions}, null,
                    ...
                    if(canRunCallback) callback();
                });
                return;
            }

            // The socket exists, but the socket state is an error or dontSaveUnsentMessage === true
            // Even if canRunCallback === false, check that callback is a function and run before delete callbackStack item
            if(typeof callback === 'function') {
                callback();
                if(sendExtOptions.sendAndReceive) callbackStack.delete(sentMessageID);
            }
            log.info('Can\'t send message to ', printConnectionInfo(socket, cfg),
                ': not connecting. ',
                (socket ?
 ...
 */
        ...
        */
        ...
        socket.on('close', function (hadError) {
            ...
            if(!log.debug('IPC client socket closed ',
                printConnectionInfo(socket, cfg, false, delayBeforeReconnect), '; had error: ', hadError,
                '; pending: ', socket.pending, '; connecting: ', socket.connecting, '; destroyed: ', socket.destroyed)
            ) {
        ...
        dataReceiver(log, socket, function (err, receivedMessageID, result) {
            ...
            if (callbackObj && typeof callbackObj.func === 'function') {
                //if(callbackObj.removed) log.info('Callback for messageID ', messageID, ', message: ', result, ' was removed at ', new Date(callbackObj.removed).toLocaleString());
                callbackObj.timestamp = Date.now();
    ...
    function saveUnsentData(data, err, callback) {
        ...
        if ((err && err.message !== prevSendErrorMessage) || savedMessagesCnt < 5 || savedMessagesCnt % 100 === 0) {
            ...
            log.info('Not connected', printConnectionInfo(socket, cfg),
        ...
        if(!bufferedData || !bufferedData.length || bufferedData.length > maxMessageLen) {
            log.warn('Can\'t save unsent data. Unsent message wrong or too long. Size ', bufferedData.length, '/ ',
                    ...
                    zlib.brotliDecompress(compressedData, function (err, stringifiedData) {
                        ...
                        async.eachSeries(unsentDataFromStorage, function (dataObj, callback) {
                            ...
                            log.info('Reading ', (err ? 'but not sending ': 'and sending '), unsentDataFromStorage.length,
                                ...
                                ': ', printConnectionInfo(socket, cfg, false));
                            if(err) log.info('Error while sending messages: ', err.message);
                            callback();
                        });
                    });
                });
                ...
                fs.close(storageFD, function (err) {
                    ...
                    fs.unlink(storageFileName, function (err) {
                        if (err || fs.existsSync(storageFileName)) {
                            log.info('Can\'t delete file ', storageFileName, ' with unsent data: ',
...
function dataSender(log, err, socket, data, sentMessageID, callback) {
    ...
    socket.write(message, function(err) {
        ...
        else if(err) log.error('Can\'t write to socket: ', err, ': msgID: ', sentMessageID, ', data: ', data);
    });
    ...
    function dataParser(data) {
        ...
        while (data.length >= headerLength) {
            ...
            try {
                ...
                //log.info('Can\'t parse JSON in received data (len: ' + data.length + '; len value from header: ' + dataLength + ') data: "' + resultStr + '" (', data,'): ' + err.message);
            ...
            //if(result.err) log.info(result.err);
...
function cleanUpCallbackStack(callbackStack, keepCallbackInterval, maxCallbacksCnt, log) {
    ...
    if(cleanUpCallbacksNum && log && typeof log.info === 'function') {
        log.info('Cleaning ' + cleanUpCallbacksNum + ' callbacks older then ' + keepCallbackInterval / 60000 +
            'min from stack');
    }
...
function socketErrorsHandler(err, socketErrorCounter, log) {
    ...
    if(socketErrorCounter.num === 1 ||
        ...
        if(socketErrorCounter.num === 1) log.info(err.message);
        else {
            log.info('Error #', socketErrorCounter.num, ', occurred from: ',
            ...
            async.each(files, function (file, callback) {
                ...
                fs.stat(filePath, function (err, stats) {
                    if(err) {
                        ...
                        log.info('Can\'t stat ', filePath, ': ', err);
                        return callback();
                    }
                    ...
                    fs.access(filePath, fs.constants.W_OK, function (err) {
                        if(err) {
                            log.info('Can\'t get access to ', filePath, ' for delete: ', err.message);
                            return callback();
                        }

                        fs.unlink(filePath, function (err) {
                            if(err) log.info('Can\'t delete unused file ', filePath, ': ', err.message);
