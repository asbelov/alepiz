...
/*
cfg = {
        localAddress: <string> - IP address for binding server
        ...
        restartListenerOnErrorTime: delay between attempts to restart the listener on error, default 5000
        socketTimeout: Sets the socket to timeout after timeout milliseconds of inactivity on the socket. default 0 - disabled
        keepAlive: delay between the last data packet received and the first keepalive probe in milliseconds
    }
...
IPC.server = function(cfg, callback) {
    ...
    else if(require.main !== module) id = 'server:' + path.basename(require.main.filename, '.js');
    else id = 'server';
    ...
    if(cfg.exitOnNoClients === parseInt(cfg.exitOnNoClients, 10) && cfg.exitOnNoClients > 1000) {
        setTimeout(function () {
            ...
            setTimeout(process.exit, 3000).unref(); // process.exit();
        }, cfg.exitOnNoClients).unref();
    }
    ...
    function send (data, socketIndex, sentMessageID, permanent, callback) {
        ...
        if(socketIndex === undefined) {
            ...
            return setTimeout(send, 1000, data, socketIndex, sentMessageID, permanent, callback).unref();
        }
    ...
    function start() {
        ...
        server = net.createServer(function(socket) {
            ...
            ++clientsNum;
            log.info('Client connected: ', socket.remoteAddress, ':', socket.remotePort, '->', localAddress, ':', serverPort);
            attemptsCnt = maxAttemptsCnt;
            ...
            dataReceiver(log, socket, function(err, receivedMessageID, result) {
                ...
                if (callbackObj && typeof callbackObj.func === 'function') {
                    ...
                    if(receivedMessageID && receivedMessageID % 2 !== 0) // for odd messageIDs
                        log.warn('IPCServer: can\'t find callback for messageID ', receivedMessageID, '/', messageID,
        ...
        server.on('error', function(err) {
            ...
            var message = 'IPC server error: ' + err.message +
                (attemptsCnt ?
                    ...
                    '. No attempts left, exiting. ' + printConnectionInfo(socket, cfg, true));
            ...
            connectAndSendMessage(serverAddress, serverPort, localAddress, stopMessage, function(err) {
                ...
                }, restartListenerOnErrorTime).unref();
            });
        });
        ...
        server.listen({
            ...
            log.info('Server starting at ', localAddress,' and bound to TCP port ', serverPort);
...
cfg = {
        ...
        reconnectDelay: delay between reconnect attempt to server on disconnect in milliseconds, default 5000, 0 - not reconnect
        connectOnDemand:if true then connect to server on demand (when required to send data). Else connect to server while initializing
        disconnectOnIdleTime: when no data sending or receiving during timeout (default 0 - dont disconnect), then disconnect from server. If required, then connect on demand
        socketTimeout: Sets the socket to timeout after timeout milliseconds of inactivity on the socket. default 0 - disabled
        connectionTimeout: Sets the connection timeout to the socket in ms. default 0 - disabled. If a reconnection timeout is specified, a reconnection attempt will be made after the connection timeout is reached
        keepAlive: delay between the last data packet received and the first keepalive probe in milliseconds
        maxReconnectAttempts: max number of reconnect before process.exit(4). If not set, used IPCClientMaxReconnectAttempts from common.json. 0 infinitely
    }
...
IPC.client = function(cfg, callback) {
    var serverAddress = cfg.serverAddress,
        ...
        keepCallbackInterval = Number(cfg.keepCallbackInterval) === parseInt(cfg.keepCallbackInterval, 10) &&
            Number(cfg.keepCallbackInterval) ? Number(cfg.keepCallbackInterval) : 1209600000, // 14 days
        maxUnsentMessagesCnt = Number(cfg.maxUnsentMessagesCnt) === parseInt(cfg.maxUnsentMessagesCnt, 10) &&
            Number(cfg.maxUnsentMessagesCnt) ? Number(cfg.maxUnsentMessagesCnt) : 5000,
        delayBeforeReconnect = Number(cfg.reconnectDelay) === parseInt(cfg.reconnectDelay, 10) ?
            Number(cfg.reconnectDelay) : 5000,
        disconnectOnIdleTime = Number(cfg.disconnectOnIdleTime) === parseInt(cfg.disconnectOnIdleTime, 10) ?
            Number(cfg.disconnectOnIdleTime) : 900000,
        connectOnDemand = cfg.connectOnDemand,
        // inactivity on the socket
        socketTimeout = Number(cfg.socketTimeout) === parseInt(cfg.socketTimeout, 10) ?
            Number(cfg.socketTimeout) : defaultSocketTimeout,
        connectionTimeout = Number(cfg.connectionTimeout) === parseInt(cfg.connectionTimeout, 10) ?
            Number(cfg.connectionTimeout) : 2000,
        maxCallbacksCnt = Number(cfg.maxCallbacksCnt) === parseInt(cfg.maxCallbacksCnt, 10) ?
            Number(cfg.maxCallbacksCnt) : 10000,
        keepAlive = Number(cfg.keepAlive) === parseInt(cfg.keepAlive, 10) && Number(cfg.keepAlive) > 1000 ?
            Number(cfg.keepAlive) : 5000,
        ...
        storageFD,
        socket,
        returnedObject = {}, //!!! dont replace to this. Object masts return by callback
        processingDataTime = Date.now(),
    ...
    if(id) id = 'client:' + id;
        ...
        } else id = 'client:' + path.basename(require.main.filename, '.js');
    } else id = 'client';
    ...
    returnedObject.send = function(data, callback) { send(data, 0, {}, callback) };

    returnedObject.sendAndReceive = function(data, callback) {
        if(typeof callback !== 'function') return log.warn('Can\' run sendAndReceive() function: callback is not set');
    ...
    returnedObject.sendExt = function(data, options, callback) {
        var myMessageID = 0;
        if(options && options.sendAndReceive) {
            messageID = getNewMessageID(messageID, firstMessageID);
            myMessageID = messageID;
        }

        send(data, myMessageID, options, callback);
    };

    returnedObject.stop = stop;
    returnedObject.destroy = disconnect;
    returnedObject.disconnect = disconnect;
    returnedObject.isConnected = function () {
    ...
    if(disconnectOnIdleTime) {
        setInterval(function () {
            if (Date.now() - processingDataTime > disconnectOnIdleTime) disconnect();
        }, disconnectOnIdleTime);
    }
    ...
    function disconnect (callback) {
        ...
        if(connectTimeoutTimer !== undefined) clearTimeout(connectTimeoutTimer);
        ...
        else if(Date.now() - processingDataTime > disconnectOnIdleTime) {
            info = ': no data ' + Math.round((Date.now() - processingDataTime) / 1000) + '/' +
                disconnectOnIdleTime / 1000 + 'sec';
        }
    ...
    function send (data, sentMessageID, options, callback) {
        //log.warn('!!!', sentMessageID, ' Send:', options, ';', data);
        processingDataTime = Date.now();

        // here try to connect. f.e. when connectOnDemand = true
        if((!socket || socket.destroyed || (socket.pending && !socket.connected)) && !stopInProgress) connect();

        // save callback before it stay null
        var savedCallback = callback;
        if(options.sendAndReceive && typeof callback === 'function') {
            ...
            callback = null;
            //log.info('Add new callback for messageID ', sentMessageID, ', options: ', options ,', message: ', data);
        }

        if(socket && !socket.pending && !socket.connecting && !socket.destroyed) {
            dataSender(log,null, socket, data, sentMessageID, function(err) {
                if(!err || options.dontSaveUnsentMessage) {
                    // run callback when no response is expected to be sent
                    // it is equal to if(!options.sendAndReceive || typeof savedCallback !== 'function') savedCallback();
                    if(typeof callback === 'function') callback(err);
                    else if(err) log.warn(err.message);
                    return;
                }

                saveUnsentData({data: data, messageID: sentMessageID, options: options}, function (err1) {
                    var commonErr = err1 ?
                    ...
                    else log.error(commonErr.message);
                });
            });
            return;
        }
        ...
        if(!options.dontSaveUnsentMessage || !socket) {
            if (savedMessagesCnt < 10 || savedMessagesCnt % 100 === 0) {
            ...
            saveUnsentData({data: data, messageID: sentMessageID, options: options}, function (err) {
            ...
            if(typeof savedCallback === 'function') {
                if(options.sendAndReceive) callbackStack.delete(sentMessageID);
                savedCallback();
            }
    ...
    function connect() {
        ...
        var maxReconnectAttempts =
            cfg.maxReconnectAttempts === undefined ?
                ...
                parseInt(cfg.maxReconnectAttempts, 10);
        if(reconnectAttempts) {
            log.warn('Reconnect to ' + serverAddress + ':' + serverPort +
        ...
        if(connectionTimeout) {
            connectTimeoutTimer = setTimeout(function () {
                // returns the IPC object when the connection timeout expires to prevent blocking of the calling function
                if(!isIPCObjectReturned) {
                    isIPCObjectReturned = true;
                    callback(null, null, returnedObject);
                }
        ...
        socket.on('connect', function() {
            ...
            //IPC server print this information before client
            if(reconnectAttempts > 1 || unsentMessagesCnt || connectOnDemand) {
                log.info('Client connecting', printConnectionInfo(socket, cfg),
            ...
            dataReceiver(log, socket, function(err, receivedMessageID, result) {
                ...
                if (callbackObj && typeof callbackObj.func === 'function') {
                    ...
                    if(receivedMessageID && receivedMessageID % 2 === 0) // for even messageIDs
                        log.warn('IPCClient: can\'t find callback for messageID ', receivedMessageID, '/', messageID,
                            ', message: "', result, '". callbackObj: ', callbackObj,
        ...
        socket.on('close', function (hadError) {
            ...
            if(delayBeforeReconnect && !reconnectTimer) {
                reconnectTimer = setTimeout(function () {
                    ...
                    // connect() comment for simple searching
                    setTimeout(connect, 0).unref();
                }, delayBeforeReconnect).unref();
            }
        });
    ...
    function saveUnsentData(data, callback) {
        ...
        try {
            var stringifiedData = Buffer.from(JSON.stringify(Array.from(unsentData)));
        ...
        if(!stringifiedData || !stringifiedData.length || stringifiedData.length > maxMessageLen) {
            log.warn('Can\'t send data. Unsent message wrong or too long: ', stringifiedData.length, '/ ',
                maxMessageLen, ': ', stringifiedData.toString('utf8', 0, 512) + '... ',
                printConnectionInfo(socket, cfg, false));
        }
    ...
    function dataParser(data) {
        ...
        while (data.length >= headerLength) {
            ...
            if (receivedMagic !== magic || receivedVersion !== version) {
                log.warn('Incorrect header, skip part of received data: "' + receivedMagic + '":' +
...
IPC.log = function(label) {
    ...
    return {
        ...
        disconnect: function (callback) { setTimeout(callback, 1000).unref(); },
    };
};
        ...
        if (logFile !== newLogFile) {
            ...
            if (newStreamLog) {
                try {
                    ...
                    newStreamLog.write('Can\'t end streamLog or close file descriptor for ' +
                        logFile + ': ' + e.message);
                }
...
function cleanUpCallbackStack(callbackStack, keepCallbackInterval, maxCallbacksCnt, log) {
    ...
    for(var [id, callbackObj] of callbackStack.entries()) {
        if(callbackObj && !callbackObj.options.permanent &&
