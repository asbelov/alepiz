...
/*
    Returned actions list with an actions properties, according user rights and selected objects
...
actions.getConfigurationForAll = function (user, objectsJSONStr, callback) {
    ...
    if(!commonActionsLayout) return callback(new Error('Error occurred, while get action menu layout from configuration'));

    // the layout of user actions is stored in the DB table actionsConfig and where actionName field is __actionsLayout
    actionClient.actionConfig(user, 'getActionConfig', '__AlepizMainMenuCustomization', null,
        function (err, row) {

        if (err) {
            return callback(new Error('Can\'t get action layout user: ' + user + ': ' + err.message));
        }
 ...
 * @param {function(Error)|function(null, Object)} callback - callback(err, actionCfg), where actionCfg -
...
actions.getConfiguration = function (actionID, callback){
    ...
    fs.readFile(actionConfigPath, 'utf8', function(err, fileBody) {
        ...
        cfg.link = path.join('/', confActions.get('dir'), actionID).replace(/\\/g, '/');
...
/*
    ...
    callback(err, description), where description is a string with an action description
