 ...
 *
 * @name checkIDs
 * @param {Array|string|number} initIDs: array of IDs, string ith comma separated IDs or single ID as number
 * @param {function(Error)|function(null, Array)} callback - callback(err, IDs) return error or array of IDs or empty array.
...
function checkIDs(initIDs, callback) {
    ...
    if(initIDs.length !== IDs.length) return callback(new Error('found duplicate or incorrect IDs in "' + String(initIDs) +
        (String(IDs) ? '", correct IDs is: "' + (String(IDs)  + '"') : '')), IDs);
