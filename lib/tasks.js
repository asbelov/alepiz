...
var rightsWrapperTaskDB = require('../rightsWrappers/tasksDB');
...
var dataFile = path.join(__dirname, '..', (conf.get('tempDir') || 'temp'),
...
tasks.startCheckConditions = function(callback) {
    history.connect('tasks', function (err) {
...
tasks.runTask = function(params, callback) {
    ...
    // [{mySessionID:.., name:<taskParameterName>, value:.., actionID:<actionID ie action dir>, actionName:<actionName>, startupOptions:<>}, ..]
        ...
        async.each(taskParameters, function(taskParameter, callback) {
            if(Array.isArray(filterSessionIDs) && filterSessionIDs.indexOf(taskParameter.sessionID) === -1) {
                //log.info('Skip mySessionID ', taskParameter.mySessionID, '(' + taskParameter.actionID+ ') in task ID ', taskID, ' ; required session: ', filterSessionIDs);
                return callback();
            }

            if(!actions[taskParameter.tasksActionsID]) {
                actions[taskParameter.tasksActionsID] = {
                    ID: taskParameter.actionID,
                    args:{},
                    startupOption: taskParameter.startupOptions,
                    sessionID: mySessionID || taskParameter.sessionID,
                    actionSessionID: taskParameter.sessionID,
                };
                actionsOrder.push(taskParameter.tasksActionsID);
            }

            replaceVariablesToValues(taskParameter.name, taskParameter.value, variables, function(err, value) {
                if(err) return callback(err);

                actions[taskParameter.tasksActionsID].args[taskParameter.name] = value;
                callback();
            });
            ...
            runTaskFunction(userName, taskID, actions, actionsOrder, function(err, returnedActionsValues) {
                ...
                rightsWrapperTaskDB.markTaskCompleted(taskID, function(_err) {
                ...
                actionFunctionOnSuccess.push(function (err, prevResult) {
                    ...
                    async.eachOf(actions[_tasksActionsID].args, function(valueWithVariables, name, callback) {
                        ...
                        runAction({
                            ...
                            if (err) return _actionFunctionOnError(err);
                            result[_actionID + ':' + _tasksActionsID] = data;
...
function getTaskParameters(username, taskID, callback) {
    rightsWrapperTaskDB.getTaskParameters(username, taskID, function(err, taskParameters, taskData, OCIDs, counters, objects){
        if(err) return callback(err);
        ...
        async.each(Object.keys(actions), function(sessionID, callback) {
            ...
            rightsWrapperTaskDB.checkActionsRights(username, Object.keys(actionsIDsObj), 'run', function(err) {
