    ...
    fs.readdir(cfg.path, function(err, logFiles){
        ...
        async.each(logFiles, function(logFile, callback){
            ...
            if(!/\.\d\d\d\d\d\d$/.test(logFile)){
                writeLog('[logRotate] file ' + logFile + ' is not a log file. Skip it', 'D');
                return callback();
            }
            ...
            fs.stat(logFile, function(err, stats){
                ...
                if(!stats.isFile()) {
                    writeLog('[logRotate] ' + logFile + ' is not a file, skip it', 'D');
                    return callback();
                }
                ...
                if(logFD[logFile]) {
                    ...
                    logFD[logFile].end('', function() {
                        ...
                        fs.access(logFile, fs.constants.W_OK, function (err) {
                            ...
                            fs.unlink(logFile, function (err) {
                                ...
                                } else writeLog('[logRotate] Can\'t remove log file ' + logFile + ': file exist after removing');
                                return callback();
                            });
                        })
                    });
                    ...
                    fs.access(logFile, fs.constants.W_OK, function (err) {
                        if (err) {
                            writeLog('[logRotate] File not opened, but can\'t remove log file ' + logFile + ': ' + err.message);
                            return callback();
                        }
                        fs.unlink(logFile, function (err) {
                            if (err) {
