...
var cfg = confLog.get(); // configuration for each module
const logDir = cfg.dir || 'logs';
var messagesCache = new Set(); // used separately in client for sending message one time per minute and in server for waiting log rotate
...
var cfgInit = {
    ...
    alwaysLogToConsoleIfLogLevelMoreThen: "E",
    exitLogFileName: 'exit.log',
    connectOnDemand: true,
    disconnectOnIdleTime: 300000,
};
var topDirName = (cfg.topDirName || cfgInit.topDirName) + path.sep;


// open stream for exit.log for write to log in any case on crashing
var logExitFilePath = path.join(String(logDir), cfg.exitLogFileName ? cfg.exitLogFileName : cfgInit.exitLogFileName);
try { fs.mkdirSync(path.dirname(logExitFilePath), {recursive: true}); } catch (e) {}
logFD.set(logExitFilePath, fs.createWriteStream(logExitFilePath, {flags: 'a'}));
...
//if (!proc.isMainThread) {
// if(!module.parent) {} === if(require.main === module) {}
if(require.main === module) {
    isThisLogServerProcess = true;

    // merge configuration from log.json with cfgInit variable
    for(var key in cfgInit) {
        if(cfg[key] === undefined) cfg[key] = cfgInit[key];
    }

    var savedCfgLogToConsole = cfg.logToConsole;
    cfg.logToConsole = cfg.lib && cfg.lib.IPC && cfg.lib.IPC.logToConsole !== undefined ? cfg.lib.IPC.logToConsole : cfg.logToConsole;
    ...
    cfg.logToConsole = savedCfgLogToConsole;
...
module.exports = function(module) {
    ...
    if(!isServerInit) {
        ...
        cfg.onStop = function(callback) {
            ...
            clientIPC.sendExt(Array.from(messagesCache), {
                sendAndReceive: true,
        ...
        var savedCfgLogToConsole = cfg.logToConsole;
        cfg.logToConsole = cfg.lib && cfg.lib.IPC && cfg.lib.IPC.logToConsole !== undefined ?
            cfg.lib.IPC.logToConsole : cfg.logToConsole;

        clientIPC = new IPC.client(cfg, function(err, message, _clientIPC) {
            if(err) writeLog(err.message);
            if(_clientIPC) {  // server fully initialized
    ...
    d.setHours(0, 0, 0, 0);
    setTimeout(setDateSuffix, d - (new Date())).unref();

    // sending messages cache to log server each 1 seconds
    var sendMessageInProgress = false;
    setInterval(function() {
        if(sendMessageInProgress || !messagesCache.size) return;
        sendMessageInProgress = true;

        var myMessagesCache = Array.from(messagesCache);
        messagesCache.clear();
...
function forkLogServer(callback) {
    new threads.parent({
        ...
        restartAfterErrorTimeout: 0, // was 500
        killTimeout: 300,
        module: 'log',
...
var log = function(module) {
    ...
    return {
        ...
        throw: function () {
            addMessageToCache('THROW', Array.prototype.slice.call(arguments), module)
        },
...
function createLabel(module) {
    ...
    if(label) {
        label = label
            .substring(
                label.toLowerCase().lastIndexOf(topDirName) + topDirName.length,
                label.lastIndexOf('.')
            ).replace(/[\\/]/g, ':');
    } else label = 'log';
...
function createConfiguration(module) {
    ...
    for(var i = labels.length-1; i !== -1; i--) {
        var additionalCfg = confLog.get(labels[i]);
        if(additionalCfg) {
...
function writeLog(messagesObj, force, _callback) {
    function callback() {
        if(!isThisLogServerProcess) {
            logFD.forEach((logFDObj, logFilePath) => {
                if(logFilePath === logExitFilePath) return;
                if(typeof logFDObj.end === 'function') logFDObj.end();
    ...
    var errorLogFilePath = path.join(String(logDir), cfg.errorLogFileName || 'error.log' + dateSuffix);
    if(!logFD.has(errorLogFilePath) && isThisLogServerProcess) {
        try { fs.mkdirSync(path.dirname(errorLogFilePath), {recursive: true}) } catch (e) {}
        logFD.set(errorLogFilePath, fs.createWriteStream(errorLogFilePath, {flags: 'a'}));
    ...
    if(typeof messagesObj !== 'object') {
        ...
        var logFilePath = path.join(String(logDir), String(force === 'EXIT' ? cfg.exitLogFileName : cfg.file + dateSuffix));

        if(!logFD.has(logFilePath)) {
            try { fs.mkdirSync(path.dirname(logFilePath), {recursive: true}); } catch (e) {}
            logFD.set(logFilePath, fs.createWriteStream(logFilePath, {flags: 'a'}));
        ...
        // always error level
        if((!force || levelOrder[force] > levelOrder.I) && logFD.has(errorLogFilePath) &&
            logFD.get(errorLogFilePath).writable) logFD.get(errorLogFilePath).write(message);

        if(logFD.has(logFilePath) && logFD.get(logFilePath).writable) logFD.get(logFilePath).write(message);
        else if(logFD.has(logExitFilePath) && logFD.get(logExitFilePath).writable) logFD.get(logExitFilePath).write(message);
        return callback();
    }
    ...
    if(isLogRotateProcessing && !force) {
        messagesObj.forEach(item => messagesCache.add(item));
        return callback();
    }
    ...
    messagesObj.forEach(function(message) {
        ...
        logFiles.forEach(function (file) {
            var logFilePath = path.join(String(logDir), String(file)) + dateSuffix;
    ...
    for(logFilePath in sortedMessagesByFiles) {
        ...
        if(logFD.has(logFilePath) && logFD.get(logFilePath).writable) logFD.get(logFilePath).write(sortedMessagesByFiles[logFilePath]);
        else {
            if(logFD.has(logExitFilePath) && logFD.get(logExitFilePath).writable) {
                logFD.get(logExitFilePath).write(sortedMessagesByFiles[logFilePath]);
            }
        }
    }
    callback();
...
function logRotate(_logDir) {
    if(!_logDir) _logDir = logDir;
    if(!Number(cfg.daysToKeepLogs) || !_logDir) return;

    writeLog('[logRotate] Log rotation started. Removing files older then ' + cfg.daysToKeepLogs + ' days from ' + _logDir, 'I');
    ...
    fs.readdir(_logDir, {withFileTypes: true}, function(err, logFiles){
        if(err) return writeLog('[logRotate] Can\'t read dir ' + _logDir + ': ' + err.message);
        ...
        async.each(logFiles, function (logFileObj, callback) {
            ...
            if(logFileObj.isDirectory()) {
                logRotate(path.join(_logDir, logFileName));
                return callback();
            }
            ...
            logFileName = path.join(_logDir, logFileName);
            ...
            writeLog('[logRotate] Log rotation is finished for ' + _logDir + '. Removed: ' + (removedLogFiles.length ? removedLogFiles.join(', ') : 'nothing'), 'I');
            setDateSuffix();

            writeLog(Array.from(messagesCache), true);
