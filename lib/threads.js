    ...
    function stopAll(callback) {
        ...
        setTimeout(function () {
            ...
            if(killedPIDs && killedPIDs.length) {
                log.error(errModuleInfo + 'Threads with TIDs ', killedPIDs.join(', '), ' were not stopped during killTimeout ',
                    options.killTimeout / 1000, 's and now killed');
                log.exit(errModuleInfo + 'Threads with TIDs ', killedPIDs.join(', '), ' were not stopped during killTimeout ',
                    options.killTimeout / 1000, 's and now killed');
            }
            if(typeof allChildrenAreStoppedCallback === 'function') {
    ...
    function runChild(callback) {
        ...
        if(treadsSpecialArgs && Array.isArray(treadsSpecialArgs)) {
            ...
            if(childIDVar === undefined) {
                if(typeof callback === 'function') {
                    return callback(new Error('Can\'t find unlinked thread arg: ' + JSON.stringify(treadsSpecialArgs)));
                } else return log.error(errModuleInfo + 'Can\'t find unlinked thread arg: ' + JSON.stringify(treadsSpecialArgs));
            }
