...
workerThreads.parent = function (options, callback) {
    ...
    function send(message, callback) {
        ...
        if(!child) return callback(new Error(errModuleInfo + 'Can\'t find working thread for send message'));

        try {
            child.postMessage({ userMessage: message });
        } catch (e) {
    ...
    function runChild(callback) {
        ...
        child.on('error', function (err) {
            if(!stopInProgress && !killInProgress) log.exit(errModuleInfo + 'worker thread return error', errRestartAfterTimeout, ': ', err.message);
            try {
                child.terminate();
                child.connected = false;
            } catch(e) {
        ...
        child.on('exit', function(exitCode) {
            ...
            if(stopInProgress || killInProgress) checkForChildrenCnt(errExitCode);
                ...
                // exitCode 10 will reserved for prevent to restart worker thread
                if(exitCode === 12) { // exitCode 12 was reserved for scheduled restart. Don't log to exit.log
                    log.warn(errModuleInfo + 'thread ', child.threadId, ' process was stopped', errExitCode, errRestartAfterTimeout);
                } else {
                    log.exit(errModuleInfo + 'thread ', child.threadId ,' process was stopped', errExitCode, exitCode !== 10 ? errRestartAfterTimeout : '');
                }
                cleanCallBackStackForPid(child.threadId);
                children.delete(child.id);
        ...
        child.on('message', function(data) {
            if(data.message === 'initComplete') {
                ...
                options.onMessage(data.userMessage, function(err, message) { // receive sendAndReceive message from thread
                    ...
                    try {
                        child.postMessage(returnedMessage);
                    } catch(e) {
...
workerThreads.child = function(options) {
    ...
    this.send = function (message, callback) {
        parentPort.postMessage({userMessage: message});
        if(typeof callback === 'function') callback();
    };
    ...
    parentPort.postMessage({ message: 'initComplete' });
    ...
    parentPort.on('message', function(data) {
        ...
        if(data.message === 'stop') {
            ...
            options.onMessage(data.userMessage, function(err, message) { // receive sendAndReceive message from parent
                ...
                parentPort.postMessage(returnedMessage);
            });
