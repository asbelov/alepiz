...
/**
 * Find objects like reObjectName. Depend of mode using SQL like or RegExp or case insensitive string compare
...
function convertToNumeric(n) {
    ...
    var n1 = n.trim();
    var res = n1.match(/^([+\-]?\d*\.?\d+(?:e[+\-]?\d+)?)(([KMGT]b)|([smhdw]))$/i); // check for abbreviation after numeric
    //log.debug(res)
    if(!res) return n; // no abbreviation after numeric
...
functions.getObjectNameLike = function(parameters, callback) {
    ...
    if(objectsNames.length !== 1) {
        ...
        // found more then one objects
        return callback(new Error('Object name like ' + parameters[0] +
            ' found more then one ('+ objectsNames.length +'); search mode ' + (parameters[1] || 'sql') +
            ': ' + objectsNames.join(',')));
    }
...
functions.getObjectNameLike.description = 'getObjectNameLike(objectName) get object name like specified\n' +
    ...
    'objectName: (SQL Like or RegExp or plain object name, mode)\n' +
    'mode: case insensitive: "sql" (default) - for sql like search, "re" - for RegExp search, "str" - for case insensitive string search\n' +
    'return the name of the object or "" if not found or throw if found more then one objects';
...
functions.getObjectID = function(parameters, callback) {
    ...
    if(objectsNames.length !== 1) {
        return callback(new Error('Object name like ' + parameters[0] +
            ' not found or found more then one ('+ objectsNames.length +'); search mode ' + (parameters[1] || 'sql') +
            ': ' + objectsNames.join(',')));
    }
...
functions.getObjectID.description = 'getObjectID(objectName) getting object ID by object name\n\n' +
    'objectName: (SQL Like or RegExp or plain object name, mode)\n' +
    'mode: case insensitive: "sql" (default) - for sql like search, "re" - for RegExp search, "str" - for case insensitive string search\n' +
    'return objectID or throw error if object not found or found more then one objects';
...
functions.getObjectProperty = function(parameters, callback) {
    ...
    if(objectsNames.length !== 1) {
        ...
        // found more then one objects
        return callback(new Error('Object name like ' + parameters[0] +
            ' found more then one ('+ objectsNames.length +'); search mode ' + (parameters[2] || 'sql') +
            ': ' + objectsNames.join(',')));
    }

    var objectProperties = cache.objectsProperties.get(objectsNamesObj[objectsNames[0]]);
    if (!Array.isArray(objectProperties) || !objectProperties.length) {
        //return callback(new Error('Not found properties for object ' + objectsNames[0]));
        return callback(null, '');
    }
    //log.warn('!!Obj: ', objectsNames[0], '; id: ', objectsNamesObj[objectsNames[0]], '; prop: ', parameters[1], '=', objectProperties);
    for(var i = 0; i < objectProperties.length; i++) {
        if(parameters[1].toUpperCase() === objectProperties[i].name.toUpperCase()) {
            return callback(null, objectProperties[i].value);
        }
    }
    ...
    /*
    objectsPropertiesDB.getObjectProperty(parameters[0], parameters[1], function (err, rows) {
        if(err) {
            return callback(new Error('Can\'t get property ' + parameters[1] +
                ' for object like ' + parameters[0] + ': ' + err.message));
        }

        return rows.length ? callback(null, rows[0].value) : callback(null, '');
    });
     */
};
