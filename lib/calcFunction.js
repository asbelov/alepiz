 ...
 * @param {Object} _cache.objectsProperties - objects properties {objectID:
 *      [objectID:, objectID:, name:, value:, description:, mode:], ...}
 */
 ...
 * @param {string} mode='sql' - case insensitive search mode: 'sql' - for sql like search, 're' - for RegExp search,
 *      'str' - for case insensitive string search
 * @returns {Object} - Object {<objectName>: <objectID>, ...} with founded object names and objectIDs or throw
 *      error message
 */
...
function getCommaSeparatedObjects (parameters, callback) {
...
functions.date2msec = function (parameters, callback){
    if(parameters && parameters[0] && typeof parameters[0] === 'string') var dateStr = parameters[0];
    else {
        var now = new Date();
...
functions.time2msec = function(parameters, callback){
    if(parameters && parameters[0] && typeof parameters[0] === 'string') var timeStr = parameters[0];
    else {
        var now = new Date();
        var time = now.getHours() * 3600000 + now.getMinutes() * 60000 + now.getSeconds() * 1000 +
            now.getMilliseconds();
...
functions.isNowTimeInInterval = function(parameters, callback){
    ...
    var timeIntervals = parameters.length === 1 ? parameters[0].split(/ *, */) : parameters;

    async.eachOf(timeIntervals, function(timeInterval, idx, callback) {
        if(typeof timeInterval !== 'string') {
            return callback(new Error('isNowTimeInInterval: error in time interval "' + String(timeInterval) +
                '" (' + parameters.join(', ') + ')'));
        }

        var interval = timeInterval.split(/ *- */);
        if(interval.length  !== 2) {
            return callback(new Error('isNowTimeInInterval: error in time interval "' + timeInterval +
                '" (' + parameters.join(', ') + ')'));
        }
...
functions.isNowTimeInInterval.description =
    'isNowTimeInInterval(<timeInterval1> [, timeInterval2, ...]) or ' +
        'for one parameter isNowTimeInInterval("<timeInterval1> [, timeInterval2, ...]") ' +
        'whether the current time is in the specified time intervals\n\n' +
...
functions.getDate = function(parameters, callback){
    var timestamp = parseInt(parameters[0], 10);
    if(parameters.length && (!timestamp || timestamp < 0)) {
        return callback(new Error('Incorrect timestamp parameter: ' + parameters[0] + ' (' + timestamp + ')'));
    }
    var now = parameters.length ? new Date(timestamp) : new Date();

    callback(null, now.getDate());
};
functions.getDate.description = 'getDate([timestamp]) return current date of month\n\n' +
    'timestamp - the timestamp for getting the function value. if not specified, the current time is taken';
...
functions.getMonth.description = 'getMonth([timestamp]) return current month number (1 - Jan... 12 - Dec)\n\n' +
    'timestamp - the timestamp for getting the function value. if not specified, the current time is taken';
...
functions.getDayOfWeek.description = 'getDayOfWeek([timestamp]) return current day of week number (1-Sun... 7-Sat)\n\n' +
    'timestamp - the timestamp for getting the function value. if not specified, the current time is taken';
...
functions.getHours.description = 'getHours([timestamp]) return current hours according to local time in (1-23)\n\n' +
    'timestamp - the timestamp for getting the function value. if not specified, the current time is taken';
...
functions.getMinutes.description = 'getMinutes([timestamp]) return current minutes according the local time (1-59)\n\n' +
    'timestamp - the timestamp for getting the function value. if not specified, the current time is taken';
...
functions.getSeconds.description = 'getSeconds([timestamp]) return current seconds according the local time (1-59)\n\n' +
    'timestamp - the timestamp for getting the function value. if not specified, the current time is taken';
...
functions.getMilliseconds.description =
    'getMilliseconds([timestamp]) return current milliseconds according the local time (1-999)\n\n' +
    'timestamp - the timestamp for getting the function value. if not specified, the current time is taken';
...
functions.getValueFromJSONStr = function(parameters, callback) {
    ...
    // slice will make copy of existing array
    var keys = parameters.length > 2 ? parameters.slice(1) : parameters[1].split(':');
    for(var i = 0; i < keys.length; i++) {
        if(obj && keys[i] && (keys[i] in obj)) obj = obj[keys[i]];
...
functions.getValueFromJSONStr.description = 'getValueFromJSONStr(JSONString, keys) or ' +
    'getValueFromJSONStr(JSONString, parentKey1, parentKey2,... parentKeyN) - getting value for a specific key from ' +
    'stringify JSON object\n\n' +

    'JSONString - Stringify JSON objects\n' +
    'keys or key1, key2,...- a string with JSON keys separated by ":" or a sequence of JSON keys listed as parameters \n' +
...
functions.testRE = function(parameters, callback) {
    ...
    try {
        ...
        return callback(new Error('Error: /' + parameters[1] + '/' + flags + '.test(' + parameters[0] + '): ' +
            err.message));
    }
...
functions.getObjectNameLike = function(parameters, callback) {
    getCommaSeparatedObjects(parameters, function (err, objectsNamesObj) {
...
functions.getCommaSeparatedObjectsIDs.description =
    'getCommaSeparatedObjectsIDs(objectNames) getting objects IDs by objects names\n' +
    '\n' +
    'objectName: (SQL Like or RegExp or plain object name, mode)\n' +
    'mode: case insensitive: "sql" (default) - for sql like search, "re" - for RegExp search, "str" - for case insensitive string search\n' +
...
functions.getObjectProperty = function(parameters, callback) {
    ...
    if(objectProperties) {
        ...
        if (objectProperty) return callback(null, objectProperty.value);
    }
    callback(null, '');
};

functions.getObjectProperty.description =
    'getObjectProperty(objectName, propertyName, mode) getting object property for specific object name\n\n' +
...
functions.getTaskResult = function(parameters, callback) {
    ...
    if(actionNumber === undefined) actionNumber = 1; // undefined or 0
        ...
        if(!actionNumber || Number(actionNumber) !== parseInt(String(actionNumber), 10))
            return callback(new Error('Incorrect action number parameter: ' + actionNumber +
                ' for getting task result from ' + parameters[0]));
    }
    ...
    var taskActionID = sessionIDs[actionNumber - 1];
    if(Number(taskActionID) !== parseInt(String(taskActionID), 10))
        return callback(new Error('Incorrect session ID "' + taskActionID + '" for getting task result from ' +
            parameters[0]));

    callback(null, taskResult[taskActionID]);
};

functions.getTaskResult.description =
    'getTaskResult(task, JSONString, actionNum) get result for specific action from task result object\n' +
...
functions.ifElse.description =
    'ifElse(condition, trueResult, falseResult) return second parameter, if first parameter is true, otherwise return third parameter\n\n' +
    'condition: condition\n' +
...
functions.isDefined = function(parameters, callback) {
    callback(null, Number(parameters.length && parameters[0] !== null && parameters[0] !== undefined));
};

functions.isDefined.description =
    'isDefined(%:?<VARIABLE>:%) return 0 if variable value is undefined or null or not present, otherwise return 1.\n' +
...
functions.calcImportant.description =
    'Calculate "important" according variable thresholds for event-generator collector\n\n' +
