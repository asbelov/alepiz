...
/**
 * Created by Alexander Belov on 26.07.2015.
 */
...
/**
 * Get collectors configuration (config.json) for all or specified collector
 * @param {string|null|undefined} collectorName collector name. if not set, then return configuration for all collectors
 * @param {function} callback - callback(err, collectors[collectorName]) or callback(err, collectors)
 * if collectorName not set where collectors is {<collectorName>: {collector configuration from config.json}>, ....},
 */
collectors.getConfiguration = function(collectorName, callback){
...
collectors.getCollectorPath = function(collectorName) {
    return path.join(__dirname, '..', confCollectors.get('dir'), collectorName);
};
...
collectors.getCollectorCode = function(collectorName, callback){
    ...
    var collectorCodePath = path.join(__dirname, '..', confCollectors.get('dir'), collectorName, 'collector.js');
...
function getCollectors(collectorName, callback) {
    ...
    var collectorsPath = path.join(__dirname, '..', confCollectors.get('dir'));
    var collectors = {};
...
function checkParameter(val, checkAs, variables, callback) {
    ...
    if(checkAs === 'uInteger'.toLowerCase()) {
        if (String(val).match(/^ *[0-9 ]+ *$/)) return callback(null, Number(val.replace(/ /g, '')));
        return callback(new Error('Value is not an unsigned integer: ' + val));
    }

    if(checkAs === 'UNZInteger'.toLowerCase()) {
    ...
    if(checkAs === 'timeInterval'.toLowerCase()) {
    ...
    if(checkAs === 'zeroOne'.toLowerCase()) {
    ...
    if(checkAs === 'tcpPort'.toLowerCase()) {
    ...
    if(checkAs === 'hostOrIP'.toLowerCase()) {
        var pattern = /^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$|^(([a-zA-Z]|[a-zA-Z][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z]|[A-Za-z][A-Za-z0-9\-]*[A-Za-z0-9])$|^(([0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,7}:|([0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,5}(:[0-9a-fA-F]{1,4}){1,2}|([0-9a-fA-F]{1,4}:){1,4}(:[0-9a-fA-F]{1,4}){1,3}|([0-9a-fA-F]{1,4}:){1,3}(:[0-9a-fA-F]{1,4}){1,4}|([0-9a-fA-F]{1,4}:){1,2}(:[0-9a-fA-F]{1,4}){1,5}|[0-9a-fA-F]{1,4}:((:[0-9a-fA-F]{1,4}){1,6})|:((:[0-9a-fA-F]{1,4}){1,7}|:)|fe80:(:[0-9a-fA-F]{0,4}){0,4}%[0-9a-zA-Z]+|::(ffff(:0{1,4})?:)?((25[0-5]|(2[0-4]|1?[0-9])?[0-9])\.){3}(25[0-5]|(2[0-4]|1?[0-9])?[0-9])|([0-9a-fA-F]{1,4}:){1,4}:((25[0-5]|(2[0-4]|1?[0-9])?[0-9])\.){3}(25[0-5]|(2[0-4]|1?[0-9])?[0-9]))$/;
...
collectors.save = function(ID, collector, code, oldID, callback) {
    ...
    var collectorPath = path.join(__dirname, '..', confCollectors.get('dir'), ID);
    ...
    fs.stat(collectorPath, function(err, stats) {
        ...
        try{
            ...
            if(oldID && oldID !== ID) {
                var oldCollectorPath = path.join(__dirname, '..', confCollectors.get('dir'), oldID);
