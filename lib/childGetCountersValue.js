...
//var serverID = Number(threads.workerData[0]);
//var childID = Number(threads.workerData[1]);
...
function startMessageListener() {
    ...
    var returnedMessages = new Set();
    var processReturnedMessages = 0;
    ...
    function sendProcessInfo() {
        ...
        sendMessageToServer.send({
            ...
            messagesQueue: returnedMessages.size,
        });
    }
    ...
    function getValue(message){
        ...
        try {
            collector(message.collectorParameters, function (err, result) {
                processCollectorResult(err, result, message);
            });
        } catch(err) {
    ...
    function processCollectorResult(err, result, message) {
        ...
        if (!preparedResult || preparedResult.value === undefined || preparedResult.value === null) {
            if(err) {
                log.options('Collector ', message.collector, ' return error and result ', result, ' for OCID: ',
                    ...
                    err.stack ? err.stack : JSON.stringify(err), '; Parameters: ', message.collectorParameters, {
                        filenames: ['counters/' + message.counterID, 'counters.log'],
                        emptyLabel: true,
                        noPID: true,
                        level: 'E'
        ...
        if (!properties || !properties.length) {
            return log.options('Received value[s] ', preparedResult.value, ' from: ',
                message.collectorParameters.$variables.OBJECT_NAME ,
        ...
        // catch it at lib/server.js
        returnedMessages.add(returnedMessage)
        if(!processReturnedMessages) sendResult();
    ...
    function sendResult() {
        processReturnedMessages = Date.now();

        var copyOfTheReturnedMessages = new Set(returnedMessages);
        returnedMessages.clear();
        var sendMessageToServer = cfg.useProcIPCForChild ? childProc : clientIPC;

        for (var returnedMessage of copyOfTheReturnedMessages) {
            sendMessageToServer.send(returnedMessage);
        }
        copyOfTheReturnedMessages.clear();
        processReturnedMessages = 0;
        if(returnedMessages.size) sendResult();
    }
                    ...
                    if(variable.objectVariable) { // objectVariable is right, not objectName
                        ...
                        if(res) {
                            ...
                            try {
                                ...
                                log.warn('Error calc objectCounterID as first parameter for history function: ', e.message,
