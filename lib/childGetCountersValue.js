...
function startMessageListener() {
    ...
    var collectorsObj = {};
    var countersObjects = {};
    var variablesDBCache = {};
    var variablesExpressionsDBCache = {};
    var objectsPropertiesDBCache = {};
    var historyFunctionList = {};
    ...
    history.getFunctionList().forEach(function (func) {
        historyFunctionList[func.name] = func.description;
    });

    cfg.serverPort = cfg.serverPortChildrenIPC;
    cfg.id  = 'getCountersValues';
    ...
    function processMessage(message, callback) {
        ...
        if (message.countersObjects || message.variables || message.variablesExpressions || message.objectsProperties) { // init cache
            if(message.countersObjects) countersObjects = message.countersObjects;
            if(message.variables) variablesDBCache = message.variables;
            if(message.variablesExpressions) variablesExpressionsDBCache = message.variablesExpressions;
            if(message.fullUpdate && message.objectsProperties) objectsPropertiesDBCache = message.objectsProperties;
            ...
            log.debug('Received cache data:',
                (message.variables ? ' history for counters: ' + Object.keys(message.variables).length : ''),
                (message.variablesExpressions ? ' expressions for counters: ' + Object.keys(message.variablesExpressions).length : ''),
                (message.objectsProperties ? ' properties for objects: ' + Object.keys(message.objectsProperties).length : ''),
                (message.countersObjects ? ' objects: ' + Object.keys(countersObjects.objects).length +
                    ', counters: ' + Object.keys(countersObjects.counters).length +
                    ', objectName2OCID: ' + Object.keys(countersObjects.objectName2OCID).length : ''));
    ...
    function getVariablesAndCheckUpdateEvents(message) {
        ...
        if (!collectorsObj[property.collector] || typeof collectorsObj[property.collector].get !== 'function') {
            log.error('Collector "', property.collector, '" undefined or object "get" is not a function (',
        ...
        getVariablesValues(property, parentVariables, updateEventState, function(err, updateEventNewState, variables, variablesDebugInfo) {
            ...
            if(err || updateEventNewState) {
                ...
                if(variablesDebugInfo)
                    dynamicLog.add('variables', objectCounterID, variablesDebugInfo);
                    //sendVariableDebugInfo(objectCounterID, variablesDebugInfo, property.debug);
                return;
            }

            if(variables && Object.keys(variables).length) {

                if(variablesDebugInfo.UPDATE_EVENT_STATE && variablesDebugInfo.UPDATE_EVENT_STATE.unresolvedVariables && variablesDebugInfo.UPDATE_EVENT_STATE.unresolvedVariables.length) {
                    log.debug('Skip update event with unresolved variables for ',
                        property.objectName, '(', property.counterName, '): ', property.expression,
                        ', unresolved variables: ', variablesDebugInfo.UPDATE_EVENT_STATE.unresolvedVariables.join(', '), ', OCID: ', property.OCID);
                    dynamicLog.add('variables', objectCounterID, variablesDebugInfo);
                    //sendVariableDebugInfo(objectCounterID, variablesDebugInfo, property.debug);
                    return;
                }
                ...
                for (var i = 0; i < counterParameters.length; i++) {
                    ...
                    if(parameter.value && typeof parameter.value === 'string') {
                        ...
                        if(res) {
                            ...
                            if (res.unresolvedVariables.length) {
                                log.error('Unresolved variables in counter parameters: ',
                                    property.objectName, '(', property.counterName, '): ', property.collector,
                                    ', Unresolved parameter: ', parameter,
                                    ', variables: ', variables, ', parameters: ', counterParameters, ', OCID: ', property.OCID);
                                dynamicLog.add('variables', objectCounterID, variablesDebugInfo);
                                //sendVariableDebugInfo(objectCounterID, variablesDebugInfo, property.debug);
                                return;
                            }
                ...
                dynamicLog.add('variables', objectCounterID, variablesDebugInfo, important);
                //sendVariableDebugInfo(objectCounterID, variablesDebugInfo, property.debug, important);
            }
    ...
    function getValue(message){
        ...
        if(!message.collector || !collectorsObj[message.collector]) {
            return log.error('Try to get value for unknown collector for ',
        ...
        try {
            collector(message.collectorParameters, function(err, result) {
                log.debug('Receiving value from OCID ', message.objectCounterID, ': ',
                    message.collectorParameters.$variables.OBJECT_NAME ,
                ...
                if (!preparedResult || preparedResult.value === undefined || preparedResult.value === null) {
                    if(err) {
                        log.error('Collector ', message.collector, ' return error and result ', result, ' for OCID: ',
                    ...
                    log.warn('Collector ', message.collector, ' return error for OCID: ', message.objectCounterID, ': ',
                        message.collectorParameters.$variables.OBJECT_NAME,
                        '(', message.collectorParameters.$variables.COUNTER_NAME,
                        '); result: ', result, '; Error: ', err.message, '; Parameters: ', message.collectorParameters);
                }
                ...
                if (!properties || !properties.length) {
                    return log.debug('Received value[s] ', preparedResult.value, ' from: ',  message.collectorParameters.$variables.OBJECT_NAME ,
            ...
            log.error('Error in collector code "', message.collectorParameters, '" for OCID ', message.objectCounterID,
                ': ',  message.collectorParameters.$variables.OBJECT_NAME ,
    ...
    function getCountersForDependedCounters(parentCounterID, parentObjectID, variables) {
        var parentCounter = countersObjects && countersObjects.counters ? countersObjects.counters[parentCounterID] : null;
        if(!parentCounter || !Object.keys(parentCounter.dependedUpdateEvents).length) return;

        var properties = [], updateEvents = parentCounter.dependedUpdateEvents;
        for(var counterID in updateEvents) {
            var counter = countersObjects.counters[counterID], updateEvent = updateEvents[counterID];
            ...
            if(updateEvent.objectFilter) {
                ...
                if(res) {
                    if (res.unresolvedVariables.length) {
                        log.warn(variables.OBJECT_NAME, '(', variables.COUNTER_NAME,
            ...
            var objectsIDs = updateEvent.parentObjectID ? Object.keys(counter.objectsIDs) : [parentObjectID];
            objectsIDs.forEach(function (objectID) {
                var objectName = countersObjects.objects[objectID];
                if(!objectName || !counter.objectsIDs[objectID] || (objectFilter && !objectFilterRE.test(objectName))) return;

                properties.push({
                    parentObjectName: countersObjects.objects[parentObjectID],
                    parentCounter : parentCounter.collector,
                    OCID: counter.objectsIDs[objectID],
        ...
        //log.warn('Returned props: ', properties, ': ', countersObjects.counters[parentCounterID]);
        return properties;
    }
 ...
 */
    function getVariablesValues(property, parentVariables, updateEventState, callback) {
        ...
        var counterID = property.counterID;
        ...
        var data = JSON.parse(JSON.stringify( {
            variables: variablesDBCache[counterID] ? variablesDBCache[counterID] : [],
            expressions: variablesExpressionsDBCache[counterID] ? variablesExpressionsDBCache[counterID] : [],
            properties: objectsPropertiesDBCache[objectID] ? objectsPropertiesDBCache[objectID] : []
        }));
        ...
        log.debug('Variables data from DB for: ', objectName, '(', counterName, '): ', data, '; init variables: ', variables);
        ...
        async.doWhilst(function (callback) {
            ...
            async.parallel([function (callback) {
                ...
                async.each(data.variables, function (variable, callback) {
                    ...
                    if(res) {
                        log.debug('Replacing variables in func parameters ', objectName,
                            '(', counterName, '): ', variable.name, ' = ', variable.function, '(', variable.functionParameters, ' => ', res.value, '); ', variables);
                        variable.functionParameters = res.value;

                        if(res.unresolvedVariables.length) return callback();
                    }

                    if(!variable.function || !historyFunctionList[variable.function])
                        return callback(new Error('Unknown history function: "' + variable.function + '" for get data for variable ' +
                            variable.name + ', ' +  objectName + '(' + counterName + ')'));
                    ...
                    if(variable.functionParameters)
                        if(typeof(variable.functionParameters) === 'string') {
                            funcParameters = variable.functionParameters.split(/[ ]*,[ ]*/).map(function (parameter) {
                                ...
                                return calc.convertToNumeric(parameter.trim());
                            });
                    ...
                    if(variable.objectVariable) { // objectVariable is right, not objectName
                        ...
                        var OCID = countersObjects.objectName2OCID[variableObjectName] ? countersObjects.objectName2OCID[variableObjectName][variable.parentCounterID] : null;
                        ...
                        if(variable.OCID) {
                            ...
                            OCID = countersObjects.counters[variable.parentCounterID] ? countersObjects.counters[variable.parentCounterID].objectsIDs[objectID] : null;
                            if(!OCID) {
                                log.warn('OCID not found (counterID: ', variable.parentCounterID, ', objectID: ', objectID, ') for history variable: ', variableObjectName, '(', variable.parentCounterName + '): ', variable.name, ' = ', variable.function, '(`', funcParameters.join('`, `'), '`), OCID:');
                                return callback();
                            }
                        }
                    }
                    ...
                    log.debug('Processing history variable: ', variableObjectName, '(', variable.parentCounterName + '): ', variable.name, ' = ', variable.function, '(`', funcParameters.join('`, `'), '`), OCID:', OCID, ' variable: ', variable);
                    ...
                    funcParameters.push(function(err, result) {
                        ...
                        log.debug('History variable value for ', objectName,
                ...
                async.each(data.properties, function (property, callback) {
                    ...
                    log.debug('Variable name for object properties: ', name);
                    if(property.mode === 3) { // it an expression, calculate it
                        calc(property.value, variables, function (err, result, functionDebug, unresolvedVariables) {
                    ...
                    } else { // it not an expression, only replace variables to values
                        ...
                        log.debug('Replacing variables ', objectName, '(', counterName, '): result: ', name, ' = ',
                ...
                async.each(data.expressions, function (variable, callback) {
                    ...
                    log.debug('Processing variable for expression: ', variableName, ' = ', variable.expression);
                    calc(variable.expression, variables, function (err, result, functionDebug, unresolvedVariables) {
                        if (!unresolvedVariables && err) return callback(err);
                        ...
                        if(unresolvedVariables) return callback();
                        ...
                        if (updateEventState === undefined || !updateEventState !== !result) {
                            ...
                            variables.UPDATE_EVENT_STATE = result; // don't replace to !!result
                            ...
                            */
                            ...
                            if(property.mode === 1 && !result ) {
                                if(!unresolvedVariables) updateEventNewState = 'Update event state was changed to false';
                                return callback();
                            }
                            if(property.mode === 4 && result) {
                                if(!unresolvedVariables) updateEventNewState = 'Update event state was changed to true';
                                return callback();
                            }

                        } else {  // here update event status is not changed
                            if(property.mode === 1 || property.mode === 2 || (property.mode === 3 && !result) || property.mode === 4) {
                                if(!unresolvedVariables) updateEventNewState = 'Update event state was not changed';
                                return callback();
                            }
                        }
                        ...
                        if(property.mode === 0 && !result) {
                            if(!unresolvedVariables) updateEventNewState = 'Update event state was changed or not changed and now it is false';
                            return callback();
                        }
            ...
            // also break calculation when update event is false
            if(!newVariables.length || updateEventNewState) return false;

            if(++attempts >= maxAttemptsToResolveVariables) {
                log.warn('Attempts: ', attempts,': ', objectName, '(', counterName,'): new\\prev unresolved variables: ', newVariables ,'\\', prevNewVariables,'; variables : ', variables, '; source data from DB: ', data);
                return false;
            }
            ...
            log.debug('Attempts: ', attempts,': ', objectName, '(', counterName,'): new\\prev variables: ', newVariables ,'\\', prevNewVariables,'; variables : ', variables, ' for data for DB: ', data);
    ...
    /*
    function sendVariableDebugInfo(OCID, variablesDebugInfo, debugEnabled, important) {
        if(typeof debugEnabled !== 'number' || debugEnabled !== 1) return;

        var sendMessageToServer = cfg.useProcIPCForChildVariableDebug ? childProc : clientIPC;
        sendMessageToServer.send({
            id: OCID,
