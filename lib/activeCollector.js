...
function startCollector(collectorName, killTimeout, onExit, callback) {
    ...
    var serverAddress = conf.get('collectors:'+collectorName+':serverAddress'),
        localAddress = conf.get('collectors:'+collectorName+':localAddress'),
        port = conf.get('collectors:'+collectorName+':port');
...
function getCollectorNamesWithSameIPAndPort(serverAddress, port) {
    ...
    var collectorsCfg = conf.get('collectors'), collectorsNames = [];
    for(var anotherCollectorName in collectorsCfg) {
        if(serverAddress === collectorsCfg[anotherCollectorName].serverAddress &&
            port === collectorsCfg[anotherCollectorName].port
